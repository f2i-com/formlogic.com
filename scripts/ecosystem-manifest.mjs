...
if (existsSync(runtimeProvenancePath)) {
  ...
  must(runtimeProvenance.nativeProtocol === 1, 'the prepared native runtime is not native hosting protocol 1');
...
if (softnProtocol) {
  must(softnProtocol.nativeProtocol === 1, `Softn's runtime speaks native protocol ${softnProtocol.nativeProtocol}; FormLogic requires 1`);
  must(softnProtocol.recordEvents === 1, `Softn's runtime record-event protocol is ${softnProtocol.recordEvents}; FormLogic requires 1`);
}
...
const manifest = {
  ...
  components: {
    ...
    softn: { pinnedBy: 'formlogic/.github/actions/prepare-hosted-runtime/action.yml', revision: softnPin, checkoutRevision: softnHead, nativeProtocol: softnProtocol?.nativeProtocol ?? null, recordEvents: softnProtocol?.recordEvents ?? null, minimumNode: softnProtocol?.minimumNode ?? null },
  ...
  compatibility: {
    hostedRuntimeProtocol: 1,
    nativeHostingProtocol: 1,
    recordEventsProtocol: 1,
...
if (check) {
  ...
  if (!same) { console.error('compatibility-manifest.json is stale: regenerate it with `node scripts/ecosystem-manifest.mjs` and commit the result'); process.exit(1); }
} else {
