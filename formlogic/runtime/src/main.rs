//! FormLogic's sandboxed evaluator.
//!
...
//! nothing a form author writes can touch the PHP worker. This binary is that
//! child. It replaces a vendored `qjs` build driven by a JavaScript harness that
//! had to capture QuickJS's `std`/`os` host modules and then `delete` them from
//! the global object before letting a guest run; zipp's `safe-sandbox` profile
//! ships no such modules, so there is nothing to remove and no list to keep in
//! step.
//!
//! ## Wire protocol (unchanged — `QuickJsRunner.php` is the other end)
//!
...
function __applyContext(ctx) {{
  ...
  for (var k in ctx) {{
    ...
    if (!__IDENT.test(k) || __DANGEROUS[k]) continue;
    globalThis[k] = ctx[k];
  }}
}}
var __replies = [];
