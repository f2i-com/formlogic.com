...
pub fn spawn_plugin(spec: &SpawnSpec<'_>, logs: LogRing) -> std::io::Result<PluginProcess> {
    ...
    // AI-405: the per-install AI-gateway token — AOKIE ONLY (never in another
    // plugin or service env). It is injected into the signed connector and
    // unlocks gateway INFERENCE routes; provider config/key routes stay on the
    // Desktop-only management plane.
    if spec.plugin_id == "aokie" {
