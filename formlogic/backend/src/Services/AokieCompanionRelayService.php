...
final class AokieCompanionRelayService
{
     ...
     */
    public function append(string $appId, string $fromParty, string $toParty, array $frames): array
    {
        $this->sweepExpired();
        ...
        $insert = $this->mysql->prepare(
            'INSERT INTO aokie_companion_relay_frames (app_id, to_party, from_party, frame)
             VALUES (?, ?, ?, ?)',
        );
        $this->mysql->beginTransaction();
        try {
            foreach ($frames as $frame) {
                $insert->execute([$appId, $toParty, $fromParty, $frame]);
            }
     ...
     * oldest first.
     *
     * @return list<array{seq:int,from:string,frame:string}>
     */
     ...
     * one compact JSON object {seq, from, frame}. The stored document is
     ...
     *
     * @param array{seq:int,from:string,frame:string} $row
     */
