...
class ChatToolsService
{
    ...
    public static function toolDefinitions(): array
    {
        ...
        $customLogic = ['type' => 'object', 'description' => "App-logic bundle: { version:1, scripts:[{ id?, hook, source, permissions?, enabled? }], permissions?:[…], connector?:{ manifest, demoDriver? } }. hook ∈ onAppStart|onScreenEnter|onScreenLeave|onButtonClick|onBeforeSubmit|onAfterSubmit|onConnectorEvent|onSyncConflict|mapConnectorDataToForm|calculateDashboardState. source = sandboxed JavaScript (≤50KB/script, ≤256KB total). permissions grant what the scripts may do (e.g. 'formlogic.responses.write', 'connector.aokie.call.answer'). connector (optional) declares a pack-embedded connector: manifest { connectorId, kind, label, commands[], journalledCommands?, demoEvents?, demoCeremonies?, captions? } + demoDriver (a sandboxed JavaScript 'function run(ctx)' simulator, ≤128KB) — the demo driver only activates with the reviewable grant 'connector.<id>.driver.demo' and only inside an explicit simulator session; real hardware always routes host-side via FormLogic Desktop."];
    ...
    public static function chatCatalog(): array
    {
        ...
        $overrides = [
            ...
            'create_flow' => "Create a FLOW (automation) in an app: a graph of nodes — LLM chat, find/submit/update records, condition, template, sandboxed JavaScript logic, HTTP, connector commands, speech — that runs when a bound trigger event fires. Set nodeCapabilities to the union of the capabilities your nodes need, e.g. ['formlogic.responses.read','formlogic.responses.write'].",
        ];
        $out = [];
