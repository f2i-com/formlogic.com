...
// 20. Package install plans (ADR-010 / PKG-106): owner-bound, expiring, single-use,
//     digest-bound review units — propose stores the exact reviewed aggregate bytes;
//     confirm claims the plan and installs those stored bytes after fresh re-resolution.
$pdo->exec("CREATE TABLE IF NOT EXISTS `package_install_plans` (
  `id` varchar(36) COLLATE utf8mb4_unicode_ci NOT NULL,
  `user_id` varchar(36) COLLATE utf8mb4_unicode_ci NOT NULL,
...
// 22. Desktop install jobs (DESK-502): the durable coordinator for work that happens on a
//     DEVICE rather than in this process. Jobs outlive a disconnect, are claimed by exactly
//     one device (claim_token proves it), and reject replay once terminal.
$pdo->exec("CREATE TABLE IF NOT EXISTS `package_install_jobs` (
  `id` varchar(36) COLLATE utf8mb4_unicode_ci NOT NULL,
  `user_id` varchar(36) COLLATE utf8mb4_unicode_ci NOT NULL,
...
$applied[] = 'package_install_jobs table ensured';

// 23. SRV-404: flow artifacts — opaque handles to binary content that must never travel inside
//     a flow's IR, node payloads, or run logs. storage_key is internal and is never projected.
$pdo->exec("CREATE TABLE IF NOT EXISTS `flow_artifacts` (
