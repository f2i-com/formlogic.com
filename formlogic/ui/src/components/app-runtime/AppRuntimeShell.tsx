...
import { ArrowLeft, Home, User, Menu, X, ChevronLeft, MoreHorizontal, WifiOff, Database, Eye, FileBarChart, Link2, PencilRuler, Rocket } from 'lucide-react';
...
export function AppRuntimeShell({ children }: AppRuntimeShellProps) {
  ...
  // Where the runtime was ENTERED from (e.g. the studio's "Use app" passes its
  // step in router state). Remembered per-slug in sessionStorage because this
  // shell (and its stores) can be torn down by internal navigation — the entry
  // origin must outlive both. Internal paths only.
  useEffect(() => {
    const state = location.state as { returnTo?: string; returnToLabel?: string } | null;
    const returnTo = state?.returnTo;
    const valid = typeof returnTo === 'string' && returnTo.startsWith('/') && !returnTo.startsWith('//');
    if (valid && appSlug) {
      try {
        sessionStorage.setItem(
          `formlogic.appEntryFrom.${appSlug}`,
          JSON.stringify({ path: returnTo, label: state?.returnToLabel ?? 'Back' })
        );
      } catch { /* storage unavailable — the Back button just won't show */ }
    }
  }, [location.state, appSlug]);
  const entryBack = useMemo<{ path: string; label: string } | null>(() => {
    const state = location.state as { returnTo?: string; returnToLabel?: string } | null;
    const returnTo = state?.returnTo;
    if (typeof returnTo === 'string' && returnTo.startsWith('/') && !returnTo.startsWith('//')) {
      return { path: returnTo, label: state?.returnToLabel ?? 'Back' };
    }
    try {
      const raw = appSlug ? sessionStorage.getItem(`formlogic.appEntryFrom.${appSlug}`) : null;
      if (raw) {
        const parsed = JSON.parse(raw) as { path?: unknown; label?: unknown };
        if (typeof parsed.path === 'string' && parsed.path.startsWith('/') && !parsed.path.startsWith('//')) {
          return { path: parsed.path, label: typeof parsed.label === 'string' ? parsed.label : 'Back' };
        }
      }
  ...
  if (hideNav) {
    ...
    return (
      <main id="app-main-content" ref={mainRef} tabIndex={-1} className="h-screen overflow-y-auto bg-gray-50 dark:bg-slate-950 outline-none">
        {children}
        {/* Chromeless draft preview: same owner-only publish reminder as the full shell,
            as a floating pill cluster so it never rearranges the custom screen. */}
        {config.app.status === 'draft' && isOwner() && (
          <div className="fixed top-3 right-3 z-40 flex items-center gap-1.5">
            {entryBack && (
              <button
                onClick={() => navigate(entryBack.path)}
                className="inline-flex h-8 items-center gap-1.5 rounded-full border border-gray-200 dark:border-slate-700 bg-white/90 dark:bg-slate-900/90 px-3 text-xs font-semibold text-gray-700 dark:text-slate-200 shadow-md backdrop-blur hover:bg-white dark:hover:bg-slate-800 transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
              >
                <ArrowLeft className="h-3.5 w-3.5" /> {entryBack.label}
              </button>
            )}
            <button
              onClick={() => navigate(`/apps/${config.app.id}/studio/publish`)}
              className="inline-flex h-8 items-center gap-1.5 rounded-full bg-primary-600 px-3 text-xs font-semibold text-primary-foreground shadow-md hover:bg-primary-700 transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
            >
              <Eye className="h-3.5 w-3.5" /> Draft preview — publish
            </button>
          </div>
        )}
  ...
  return (
    <div className="h-dvh flex bg-gray-50 dark:bg-slate-950">
      ...
      <aside className={cn(
        ...
        <nav className="scrollbar-thin flex-1 p-2 overflow-y-auto">
      ...
      <div className="flex-1 flex flex-col min-h-0 min-w-0">
        ...
        {/* Draft preview banner — the server only serves a draft runtime to its
            owner, so this doubles as the "publish before sharing" reminder. */}
        {config.app.status === 'draft' && isOwner() && (
          <div role="status" className="bg-primary-50 dark:bg-primary-500/10 border-b border-primary-200/70 dark:border-primary-500/20 px-4 py-2 flex flex-wrap items-center gap-x-3 gap-y-1.5">
            {entryBack && (
              <button
                onClick={() => navigate(entryBack.path)}
                className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-primary-200 dark:border-primary-500/25 bg-white/70 dark:bg-white/[0.04] px-2.5 text-xs font-semibold text-primary-700 dark:text-primary-300 hover:bg-white dark:hover:bg-white/[0.08] transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
              >
                <ArrowLeft className="h-3.5 w-3.5" /> {entryBack.label}
              </button>
            )}
            <span className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-primary-700 dark:text-primary-300">
              <Eye className="h-3.5 w-3.5" /> Draft preview
            </span>
            <p className="text-xs sm:text-sm text-primary-800 dark:text-primary-200 min-w-0 flex-1">
              <span className="sm:hidden">Only you can see this app until you publish.</span>
              <span className="hidden sm:inline">Only you can see this app right now. Publish it when you're ready to share.</span>
            </p>
            <button
              onClick={() => navigate(`/apps/${config.app.id}/studio/publish`)}
              className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-primary-600 px-3 text-xs font-semibold text-primary-foreground hover:bg-primary-700 transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
            >
