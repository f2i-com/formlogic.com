...
export function NativeEditor({ app, onClose, onInstalled, initialTab = 'project', initialTable = '' }: { app: { id: string; slug: string; name: string }; onClose: () => void; onInstalled?: (home: boolean) => void; initialTab?: 'project'|'backend'|'records'; initialTable?: string }) {
  ...
  return <Modal isOpen title="Native app hosting" size="2xl" onClose={close} footer={<div className="flex flex-wrap items-center justify-between gap-3"><span className="text-xs text-slate-500 dark:text-slate-400">{version ? `Installed version ${version}` : 'No native app installed'}{dirty ? ' · Unsaved draft' : ''}</span>{tab === 'records' ? <Button variant="secondary" onClick={close}>Done</Button> : <Button disabled={!ready || !available || !project || !dirty || busy} isLoading={busy} onClick={() => void save()}>{version ? 'Publish changes' : 'Install app project'}</Button>}</div>}>
    <div className="space-y-5 p-4 sm:p-6">
      ...
      {tab === 'screens' && <div className="space-y-3"><p className="text-sm leading-6 text-slate-600 dark:text-slate-400">Edit the interface and its client logic. Download the editable project to use the full visual Builder or Studio, then import your changes here.</p>{project && <><select aria-label="Interface file" className={control} value={activeScreen} onChange={event => setScreenFile(event.target.value)}>{screenFiles.map(path => <option key={path}>{path}</option>)}</select><textarea aria-label="Interface source" className={`${control} min-h-80 font-mono text-sm`} spellCheck={false} disabled={busy} value={project.files[activeScreen] || ''} onChange={event => { setProject({ ...project, files: { ...project.files, [activeScreen]: event.target.value } }); setDirty(true); }} /></>}</div>}
      {tab === 'backend'  && <div className="space-y-3"><p className="text-sm leading-6 text-slate-600 dark:text-slate-400">Edit the app’s private .logic source. Installed migrations are read-only; import a new numbered migration to change the schema. Publishing preserves existing records.</p>{project && <><select aria-label="Private backend file" className={control} value={sourceFile} onChange={event => setSourceFile(event.target.value)}>{backendFiles.map(path => <option key={path}>{path}</option>)}</select><textarea aria-label="Private backend source" className={`${control} min-h-80 font-mono text-sm`} spellCheck={false} disabled={busy} readOnly={!sourceFile.endsWith('.logic')} value={project.files[sourceFile] || ''} onChange={event => { setProject({ ...project, files: { ...project.files, [sourceFile]: event.target.value } }); setDirty(true); }} /></>}</div>}
