...
export function NativeRecordsBrowser({ appId, version, initialTable = '' }: { appId: string; version: number; initialTable?: string }) {
  ...
  const page = Math.floor(effectiveOffset / 50) + 1;
  ...
  const pagination = <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4 dark:border-slate-700">
    ...
    <div className="flex gap-2"><Button variant="secondary" className="min-h-11" disabled={loading || effectiveOffset === 0} onClick={() => setOffset(Math.max(0, effectiveOffset - 50))} leftIcon={<ChevronLeft className="h-4 w-4" />}>Previous</Button><Button variant="secondary" className="min-h-11" disabled={!canGoNext} onClick={() => setOffset(effectiveOffset + 50)} rightIcon={<ChevronRight className="h-4 w-4" />}>Next</Button></div>
  </div>;
  return <section aria-label="Database records browser" className="space-y-4">
    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900">
      <div className="flex items-center gap-2 text-sm font-semibold text-slate-900 dark:text-white"><Database className="h-4 w-4" />App database<span className="ml-auto rounded-full border border-slate-300 px-2 py-1 text-xs font-normal text-slate-600 dark:border-slate-600 dark:text-slate-300">Owner controls</span></div>
      <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">Browse and manage the records used by this app. Open a record to edit its fields or delete it, or add a new record to the selected table.</p>
    </div>
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
      <label className="block min-w-0 flex-1 text-sm font-medium text-slate-800 dark:text-slate-200">Table<select aria-label="Database table" className={`${control} mt-2`} value={table} disabled={loading && !records} onChange={event => { setTable(event.target.value); setNotice(''); setOffset(0); setRecords(current => current ? { tables: current.tables } : null); }}><option value="">Choose a table ({records?.tables.length ?? 0})</option>{records?.tables.map(name => <option key={name} value={name}>{name}</option>)}</select></label>
    ...
    {editor && records?.schema && <NativeRecordEditor appId={appId} table={table} recordKey={editor.key} fields={records.schema.fields} onClose={() => setEditor(null)} onSaved={message => { setEditor(null); setNotice(message); setOffset(0); setReload(value => value + 1); }} />}
