...
export function NativeDatabaseTables({ app, database }: {
  ...
  return <section aria-label="App database tables" className="col-span-full min-w-0 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900/50 sm:p-5">
    ...
    {database.readOnly && <p role="note" className="mt-3 rounded-xl bg-slate-50 p-3 text-sm leading-6 text-slate-700 dark:bg-slate-800/60 dark:text-slate-300">This is the shared demo, so the app database is read-only: browse its tables and records; changes are disabled.</p>}
    {database.error && <p role="alert" className="mt-3 text-sm text-red-700 dark:text-red-300">{database.error}</p>}
    {database.installed && <>
