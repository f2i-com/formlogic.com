...
import { ShieldCheck, ShieldAlert, ShieldQuestion, Cpu, Plug, KeyRound, LayoutGrid, Code2 } from 'lucide-react';
import { Badge } from '../ui/Badge';
import type { PackCapabilitySummary } from '../../lib/api';
...
/**
 * Capability review panel (spec §30.1): what the package can do, surfaced from the server's
 * PackCapabilities describe() BEFORE the user installs. Read-only summary; no consent logic here.
 */
export function CapabilityReview({ caps, trust }: { caps: PackCapabilitySummary; trust?: string | null }) {
    ...
    <div className="rounded-lg border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 space-y-2">
      ...
      {caps.permissions.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {caps.permissions.map((p) => (
            <span key={p} className="rounded bg-amber-50 dark:bg-amber-500/10 px-1.5 py-0.5 text-[11px] font-medium text-amber-700 dark:text-amber-300">{p}</span>
          ))}
        </div>
      )}
    </div>
