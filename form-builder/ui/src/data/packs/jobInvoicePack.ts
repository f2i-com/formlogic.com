    ...
    // ── 1. Client ─────────────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var rec='';for(var j=0;j<Math.min(6,rows.length);j++){var a=rows[j].answers||{};rec+=rowItem(h(a.business_name||a.contact_name||'Client'),h(labelFor(tMap,a.client_type))+' \\u00b7 '+h(fmtDate(rows[j].submittedAt)),a.client_since?'<span class=\"chip\">since '+h(fmtDate(a.client_since))+'</span>':'');}",
    ...
    // ── 3. Quote ──────────────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var rows=await recs(500);",
          "var sMap=optionMap(ctx.fields,'status');",
          "var nSent=0,vSent=0,vTot=0,vAcc=0;for(var i=0;i<rows.length;i++){var a0=rows[i].answers||{};vTot+=num(a0.total);if(a0.status==='sent'){nSent++;vSent+=num(a0.total);}if(a0.status==='accepted')vAcc+=num(a0.total);}",
          "var html='<div class=\"wrap\">';",
          ...
          "var bd=breakdown(rows,'status',sMap)||emptyBlock(I.chart,'Quote statuses chart here once quotes go out.','+ Issue a quote');",
          "var rec='';for(var j=0;j<Math.min(6,rows.length);j++){var a=rows[j].answers||{};rec+=rowItem(h(a.quote_number||'Quote'),h(labelFor(sMap,a.status))+' \\u00b7 '+h(fmtDate(a.issue_date||rows[j].submittedAt)),'<span class=\"amt\">'+mnyC(a.total)+'</span>'+(a.valid_until?'<span class=\"chip\">valid to '+h(fmtDate(a.valid_until))+'</span>':''));}",
    ...
    // ── 4. Invoice ────────────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var rows=await recs(500);",
          "var sMap=optionMap(ctx.fields,'status');",
          "var outst=0,openN=0,ovN=0,coll=0;",
          "for(var i=0;i<rows.length;i++){var a0=rows[i].answers||{};if(a0.status==='paid'){coll+=num(a0.amount);}else{coll+=num(a0.amount_paid);}if(QC[a0.status]){outst+=Math.max(0,num(a0.amount)-num(a0.amount_paid));openN++;if(a0.status==='overdue')ovN++;}}",
          "var att=[];for(var t=0;t<rows.length;t++){if(QC[(rows[t].answers||{}).status])att.push(rows[t]);}",
          "att.sort(function(x,y){var ax=x.answers||{},ay=y.answers||{};var ox=ax.status==='overdue'?0:1,oy=ay.status==='overdue'?0:1;if(ox!==oy)return ox-oy;var dx=pd(ax.due_date),dy=pd(ay.due_date);return (dx?dx.getTime():9e15)-(dy?dy.getTime():9e15);});",
          "var qh='';for(var q=0;q<att.length&&q<6;q++){var aq=att[q].answers||{};var bal=Math.max(0,num(aq.amount)-num(aq.amount_paid));var dd=dayDiff(aq.due_date);qh+=qRow(QC[aq.status],h(aq.invoice_number||'Invoice'),h(labelFor(sMap,aq.status))+' \\u00b7 due '+h(fmtDate(aq.due_date))+(dd!=null&&dd<0?' \\u00b7 '+(-dd)+'d late':''),'<span class=\"amt\">'+mnyC(bal)+'</span>');}",
          "var qB=qh||(rows.length?emptyBlock(I.check,'Nothing needs chasing \\u2014 no sent, part-paid, or overdue invoices right now.','+ New invoice'):emptyBlock(I.doc,'Unpaid and overdue invoices queue here so nothing slips.','+ Raise your first invoice'));",
          "var rec='';for(var j=0;j<Math.min(6,rows.length);j++){var a=rows[j].answers||{};rec+=rowItem(h(a.invoice_number||'Invoice'),'Issued '+h(fmtDate(a.issue_date||rows[j].submittedAt)),'<span class=\"amt\">'+mnyC(a.amount)+'</span>'+pill(labelFor(sMap,a.status),ivK(a.status)));}",
  ...
  apps: [
    {
      ...
      customScreen: {
        ...
        js: [
          ...
          "function isOD(a){if(a.status==='overdue')return true;if(a.status!=='sent'&&a.status!=='part-paid')return false;var ddx=dayDiff(a.due_date);return ddx!=null&&ddx<0;}",
          "var outstanding=0,nUnpaid=0,nOverdue=0;for(var i2=0;i2<invoices.length;i2++){var a2=invoices[i2].answers||{};if(a2.status==='sent'||a2.status==='part-paid'||a2.status==='overdue'){outstanding+=Math.max(0,num(a2.amount)-num(a2.amount_paid));nUnpaid++;if(isOD(a2))nOverdue++;}}",
          ...
          "var att=[];for(var i6=0;i6<invoices.length;i6++){var a6=invoices[i6].answers||{};var dd6=dayDiff(a6.due_date);if(isOD(a6))att.push({a:a6,dd:dd6,bad:true});else if((a6.status==='sent'||a6.status==='part-paid')&&dd6!=null&&dd6<=7)att.push({a:a6,dd:dd6,bad:false});}",
          ...
          "else if(att.length){for(var i7=0;i7<Math.min(6,att.length);i7++){var t7=att[i7];var rem7=Math.max(0,num(t7.a.amount)-num(t7.a.amount_paid))||num(t7.a.amount);attB+=qRow(t7.bad?'var(--fl-bad)':'var(--fl-warn)',h(t7.a.invoice_number||'Invoice'),h(refName(cMap,t7.a.client)||'\\u2014'),'<span class=\"amt\">'+mnyC(rem7)+'</span>'+(dueTxt(t7.dd)?'<span class=\"chip\">'+h(dueTxt(t7.dd))+'</span>':''));}}",
          "else{var iStat=optionMap(fI,'status');var rws='';for(var i8=0;i8<Math.min(5,invoices.length);i8++){var a8=invoices[i8].answers||{};rws+=rowItem(h(a8.invoice_number||'Invoice'),h(refName(cMap,a8.client)||'\\u2014'),'<span class=\"amt\">'+mnyC(a8.amount)+'</span>'+pill(labelFor(iStat,a8.status),a8.status==='paid'?'good':a8.status==='part-paid'?'warn':a8.status==='sent'?'accent':'neutral'));}attB='<div class=\"okline\">All clear \\u2014 nothing needs chasing</div><div class=\"rows\">'+rws+'</div>';}",
