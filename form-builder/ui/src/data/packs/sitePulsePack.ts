    ...
    // ── 3. Subcontractor ────────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "  var html='<div class=\"wrap\">';",
          "  html+=headerBlock(GL,ctx.title||'Subcontractors',brief(['<b>'+fmtInt(approved)+'</b> approved',expiring>0?'<b>'+fmtInt(expiring)+'</b> insurance expiring':'',trades>0?'<b>'+fmtInt(trades)+'</b> trade'+(trades===1?'':'s')+' covered':'']),'Add subbie');",
          "  html+='<div class=\"kpis\">'",
          "    +kpi('Approved',approved,'of <b>'+fmtInt(rows.length)+'</b> on the register',{hero:true,spark:spark(weekly(rows))})",
          "    +kpi('Insurance expiring',expiring,expiring>0?'<span style=\"color:var(--fl-warn);font-weight:650\">chase certificates</span>':'all cover current')",
  ...
  apps: [
    {
      ...
      customScreen: {
        ...
        js: [
          ...
          "  for(var e0=0;e0<delivs.length;e0++){var ea=delivs[e0].answers||{};var ed=dayDiff(ea.delivery_date);if(ed!=null&&ed>=-6&&ed<=0)delWeek++;if(ed!=null&&ed>=-30&&ed<=0&&ea.issues&&String(ea.issues).trim())delIssues++;}",
