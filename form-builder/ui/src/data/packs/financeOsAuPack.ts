    ...
    // ── 2. Risk Profile Assessment ──────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var n=rows.length,tot=0,gro=0;",
          "for(var i=0;i<n;i++){var a=rows[i].answers||{};tot+=num(a.risk_profile_score);var p=String(a.recommended_profile||'');if(p==='growth'||p==='high_growth')gro++;}",
          "var avg=n?Math.round(tot/n):0,pctG=n?Math.round(gro/n*100):0;",
          ...
          "var rec='';",
          "for(var r=0;r<Math.min(6,n);r++){var rr=rows[r],ra=rr.answers||{};var sc=Math.round(num(ra.risk_profile_score));rec+=rowItem(h(labelFor(profMap,ra.recommended_profile))+' profile',h(labelFor(expMap,ra.investment_experience))+' experience \\u00b7 '+h(fmtDate(rr.submittedAt)),'<span class=\"amt\">'+(sc||'\\u2014')+'</span><span class=\"chip\">score</span>');}",
          "var recBody=rec||emptyBlock(I.user,'Completed assessments appear here with profile and score.','+ New assessment');",
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(GLY,ctx.title||'Risk Profile Assessment',brief(['<b>'+n+'</b> assessments','avg score <b>'+(n?avg:'\\u2014')+'</b>','<b>'+gro+'</b> growth-oriented']),'New assessment');",
          "html+='<div class=\"kpis\">'+kpi('Assessments',n,n?'suitability profiles on file':'Run your first assessment',{hero:true,spark:spark(weekly(rows))})+kpi('Avg risk score',n?String(avg):'\\u2014',n?'weighted score out of <b>100</b>':'No assessments yet')+kpi('Growth-oriented',gro,n?'<b>'+pctG+'%</b> Growth or High Growth':'Corporations Act s961B')+'</div>';",
    ...
    // ── 4. Financial Services Guide Acknowledgement ──────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var mBody='';",
          "if(mTot){for(var m3=5;m3>=0;m3--){var k3=curKey-m3;var lb=new Date(Math.floor(k3/12),k3%12,1).toLocaleDateString(undefined,{month:'short'});mBody+=barRow(lb,mCnt[k3]||0,mMax);}}",
          "mBody=mBody||emptyBlock(I.chart,'Signing activity by month appears here.','+ New acknowledgement');",
          "var rec='';",
          "for(var r=0;r<Math.min(6,n);r++){var rr=rows[r],ra=rr.answers||{};rec+=rowItem('FSG acknowledged','Signed '+h(fmtDate(ra.acknowledgement_date||rr.submittedAt))+' \\u00b7 '+h(ago(rr.submittedAt)),pill('Signed','good'));}",
    ...
    // ── 5. Annual Client Review ──────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var rec='';",
          "for(var r=0;r<Math.min(6,n);r++){var rr=rows[r],ra=rr.answers||{};rec+=rowItem(mnyC(ra.current_portfolio_value)+' portfolio','Reviewed '+h(fmtDate(rr.submittedAt))+' \\u00b7 fee '+mnyC(ra.current_annual_fee),'<span class=\"amt\">'+Math.round(num(ra.goal_progress))+'/10</span><span class=\"chip\">goals</span>');}",
          ...
          "html+='<div class=\"kpis\">'+kpi('Reviews',n,over?'<b>'+over+'</b> overdue':(n?'annual reviews completed':'Book your first review'),{hero:true,spark:spark(weekly(rows))})+kpi('Due within 60 days',due60,over?'<b>'+over+'</b> already overdue':'review pipeline')+kpi('Portfolio value',n?mnyC(pv):'\\u2014',n?'across <b>'+fmtInt(n)+'</b> reviews':'No reviews yet')+'</div>';",
    ...
    // ── 6. Fee Disclosure Statement ──────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var n=rows.length,fua=sumBy(rows,'portfolio_value'),fees=sumBy(rows,'calculated_annual_fee'),cons=0;",
          "for(var i=0;i<n;i++){if(String((rows[i].answers||{}).ongoing_fee_consent||'')==='yes')cons++;}",
          ...
          "var rec='';",
          "for(var r=0;r<Math.min(6,n);r++){var rr=rows[r],ra=rr.answers||{};rec+=rowItem(mnyC(ra.portfolio_value)+' FUA',h(labelFor(feeMap,ra.fee_type))+' \\u00b7 '+h(labelFor(freqMap,ra.billing_frequency))+' \\u00b7 effective '+h(fmtDate(ra.effective_date)),'<span class=\"amt\">'+mnyC(ra.calculated_annual_fee)+'</span>'+(String(ra.ongoing_fee_consent||'')==='yes'?pill('Consented','good'):pill('No consent','warn')));}",
    ...
    // ── 7. Document Vault ────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "html+='<div class=\"kpis\">'+kpi('Documents on file',n,n?'SOAs, PDSs, tax and insurance':'Add your first document',{hero:true,spark:spark(weekly(rows))})+kpi('Expiring within 60 days',exp60,expired?'<b>'+expired+'</b> already expired':'insurance, PDS and SOA')+kpi('Statements of Advice',soa,n?'<b>'+pds+'</b> PDSs on file':'No documents yet')+'</div>';",
    ...
    // ── 9. Binding Death Benefit Nomination ──────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var nmF=['primary_name_1','primary_name_2','primary_name_3','contingent_name_1','contingent_name_2'];",
          "var n=rows.length,lap6=0,lapsed=0,benes=0;",
          "for(var i=0;i<n;i++){var a=rows[i].answers||{};var dd=dayDiff(a.nomination_date);if(dd!=null){var dtl=dd+1095;if(dtl<0)lapsed++;if(dtl<=180)lap6++;}for(var f2=0;f2<nmF.length;f2++){if(String(a[nmF[f2]]||'').trim())benes++;}}",
          "var relF=['primary_relationship_1','primary_relationship_2','primary_relationship_3'];",
          "var cnt={};for(var c1=0;c1<relF.length;c1++){var cb=countBy(rows,relF[c1]);for(var k1 in cb){cnt[k1]=(cnt[k1]||0)+cb[k1];}}",
    ...
    // ── 10. Superannuation Rollover ──────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var rec='';",
          "for(var r=0;r<Math.min(6,n);r++){var rr=rows[r],ra=rr.answers||{};rec+=rowItem(h(ra.from_fund_name||'Fund')+' \\u203a '+h(ra.to_fund_name||'Fund'),h(labelFor(presMap,ra.preservation_status))+' \\u00b7 '+h(fmtDate(rr.submittedAt)),'<span class=\"amt\">'+mnyC(mvAmt(ra))+'</span>'+pill(labelFor(rtMap,ra.rollover_type),String(ra.rollover_type||'')==='full'?'accent':'neutral'));}",
