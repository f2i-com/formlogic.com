    ...
    // ── 3. Maintenance Request ────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "for(var i=0;i<rows.length;i++){var a0=rows[i].answers||{};if(OPEN[a0.status]){open.push(rows[i]);if(a0.priority==='urgent')urgent++;}var dd=dayDiff(rows[i].submittedAt);if(dd!=null&&dd>=-7)week++;}",
    ...
    // ── 4. Work Order ─────────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "function schRow(t){var at=t.answers||{};var dd2=dayDiff(at.scheduled_date);var wc=num(at.cost);return '<div class=\"sch\"><span class=\"sch-t\">'+(dd2===0?'Today':h(fmtDate(at.scheduled_date)))+'</span><div class=\"row-main\"><div class=\"row-title\">'+h(at.assigned_to||'Unassigned')+'</div><div class=\"row-sub\">'+h(labelFor(stMap,at.status))+(num(at.hours)>0?' \\u00b7 '+h(String(num(at.hours)))+'h':'')+'</div></div>'+(wc>0?'<div class=\"row-r\"><span class=\"amt\">'+mny(wc)+'</span></div>':'')+'</div>';}",
          ...
          "for(var r=0;r<rr.length;r++){var a=rr[r].answers||{};var rc=num(a.cost);rec+=rowItem(h(a.assigned_to||'Unassigned'),h(labelFor(stMap,a.status))+' \\u00b7 '+h(fmtDate(a.scheduled_date||rr[r].submittedAt)),rc>0?'<span class=\"amt\">'+mny(rc)+'</span>':'');}",
          ...
          "html+=kpis;",
          "html+=sec('Schedule')+'<div class=\"grid g21\">'+panel('Upcoming work',schHtml)+panel('By status',stHtml)+'</div>';",
          "html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
  ...
  apps: [
    {
      ...
      customScreen: {
        ...
        js: [
          ...
          "var woHtml='';",
          "for(var v0=0;v0<Math.min(ups.length,5);v0++){var wr=ups[v0],wa=wr.answers||{};var rid=Array.isArray(wa.request)?wa.request[0]:wa.request;var paddr=rid?(reqProp[rid]||''):'';var wsub=h(labelFor(woStat,wa.status))+(paddr?' \\u00b7 '+h(paddr):'');var wdd=dayDiff(wa.scheduled_date);var when=wdd==null?fmtDate(wa.scheduled_date):(wdd===0?'today':(wdd>0?'in '+wdd+'d':fmtDate(wa.scheduled_date)));var wc=num(wa.cost);woHtml+=rowItem(h(wa.assigned_to||'Unassigned'),wsub,(wc>0?'<span class=\"amt\">'+mny(wc)+'</span>':'')+'<span class=\"chip\">'+h(when)+'</span>');}",
