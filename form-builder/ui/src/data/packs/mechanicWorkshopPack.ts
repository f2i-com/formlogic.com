    ...
    // ── 2. Vehicle ────────────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var ySum=0,yN=0;for(var i=0;i<rows.length;i++){var y=num((rows[i].answers||{}).year);if(y>1900){ySum+=y;yN++;}}",
    ...
    // ── 3. Job Card ───────────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var prOrd={urgent:0,high:1,normal:2,low:3};",
          "open.sort(function(a,b){var pa=prOrd[(a.answers||{}).priority],pb=prOrd[(b.answers||{}).priority];pa=pa==null?2:pa;pb=pb==null?2:pb;if(pa!==pb)return pa-pb;return String((a.answers||{}).date_in||'').localeCompare(String((b.answers||{}).date_in||''));});",
    ...
    // ── 5. Invoice ────────────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var recent='';",
          "for(var r=0;r<rows.length&&r<6;r++){var a2=rows[r].answers||{};var k=a2.status;var kind=k==='paid'?'good':k==='overdue'?'bad':k==='sent'?'accent':'neutral';recent+=rowItem(h(a2.invoice_number||'Invoice'),'Issued '+h(fmtDate(a2.issue_date||rows[r].submittedAt)),'<span class=\"amt\">'+mny(a2.total)+'</span>'+pill(labelFor(stMap,k),kind));}",
