    ...
    // ── 3. Purchase Order ─────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "  for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};if(a.status==='draft'||a.status==='ordered'||a.status==='part-received'){open.push(rows[i]);openTotal+=num(a.total);if(a.status!=='draft'){var od=dayDiff(a.expected_date);if(od!=null&&od<0)overdueCt++;}}}",
          "  open.sort(function(x,y){function key(r){var a0=r.answers||{};if(a0.status==='draft')return 9e15;var d0=pd(a0.expected_date);return d0?d0.getTime():8e15;}return key(x)-key(y);});",
          "  function stKind(s){return s==='ordered'?'accent':s==='part-received'?'warn':s==='received'?'good':s==='cancelled'?'bad':'neutral';}",
          "  var qRows='';",
          "  for(var k=0;k<Math.min(open.length,6);k++){var oa=open[k].answers||{};var isDr=oa.status==='draft';var dd=isDr?null:dayDiff(oa.expected_date);var col=isDr?'var(--fl-muted)':dd!=null&&dd<0?'var(--fl-bad)':dd!=null&&dd<=7?'var(--fl-warn)':'var(--fl-accent)';var when=isDr?'Drafted '+fmtDate(oa.order_date)+' \\u00b7 not yet ordered':oa.expected_date?(dd!=null&&dd<0?'Expected '+fmtDate(oa.expected_date)+' \\u00b7 '+(-dd)+'d overdue':'Expected '+fmtDate(oa.expected_date)+' \\u00b7 '+ago(oa.expected_date)):'Ordered '+fmtDate(oa.order_date)+' \\u00b7 no delivery date';qRows+=qRow(col,h(oa.po_number||'Purchase order'),h(when),pill(labelFor(stMap,oa.status),stKind(oa.status))+'<span class=\"amt\">'+mnyC(oa.total)+'</span>');}",
    ...
    // ── 4. PO Line Item ───────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "function unitVal(a){var q=num(a.quantity),lt=num(a.line_total);if(lt>0&&q>0)return lt/q;return num(a.unit_cost);}",
          "function volRow(label,count,max){if(count>0)return barRow(label,count,max);return '<div class=\"bar-row\"><span class=\"bar-name\" title=\"'+h(label)+'\">'+h(label)+'</span><div class=\"bar-track\"><div class=\"bar-fill\" data-pct=\"0\"></div></div><span class=\"bar-val\">'+fmtInt(count)+'</span></div>';}",
          "async function main(){",
          "  var root=document.getElementById('app');",
          "  var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "  var rows=await recs(500);",
          ...
          "  var wBars='';",
          "  if(wTot>0){for(var b=0;b<8;b++){wBars+=volRow(b===0?'This week':b===1?'Last week':b+'w ago',wk[b],wMax);}}",
          "  var volume=wBars||emptyBlock(I.chart,'Add line items to see weekly ordering volume.','+ Add a line item');",
          "  var rRows='';",
          "  for(var r2=0;r2<Math.min(rows.length,6);r2++){var a2=rows[r2].answers||{};rRows+=rowItem(fmtInt(a2.quantity)+' \\u00d7 '+mny2(unitVal(a2)),'Added '+fmtDate(rows[r2].submittedAt),'<span class=\"amt\">'+mnyC(lineVal(a2))+'</span>');}",
  ...
  apps: [
    {
      ...
      customScreen: {
        ...
        js: [
          ...
          "  var poRows='';",
          "  for(var k=0;k<Math.min(poList.length,6);k++){var pr=poList[k].answers||{};var sn=refName(supName,pr.supplier);var ttl=h(pr.po_number||'PO')+(sn?' \\u00b7 '+h(sn):'');var ex;if(pr.status==='draft'){ex='Drafted '+fmtDate(pr.order_date)+' \\u00b7 not yet ordered';}else if(pr.expected_date){var dd=dayDiff(pr.expected_date);ex='Expected '+fmtDate(pr.expected_date)+(dd==null?'':dd<0?' \\u00b7 overdue':' \\u00b7 '+ago(pr.expected_date));}else{ex='Ordered '+fmtDate(pr.order_date);}poRows+=rowItem(ttl,'<span class=\"chip\">'+h(ex)+'</span>',pill(labelFor(stMap,pr.status),poKind(pr.status))+'<span class=\"amt\">'+mnyC(pr.total)+'</span>');}",
          "  var swRows='';",
