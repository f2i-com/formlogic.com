    ...
    // ── 1. Job Application ────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "  var rec='';",
          "  for(var r=0;r<Math.min(6,rows.length);r++){var a=rows[r].answers||{};var nm=((a.first_name||'')+' '+(a.last_name||'')).trim()||'Candidate';var right=num(a.salary_expectations)>0?'<span class=\"amt\">'+mnyC(a.salary_expectations)+'</span>':'<span class=\"chip\">'+h(ago(rows[r].submittedAt))+'</span>';rec+=rowItem(h(nm),h(labelFor(posMap,a.position))+' \\u00b7 '+h(fmtDate(rows[r].submittedAt)),right);}",
    ...
    // ── 3. Employee Onboarding ────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "  var soon=0,pol=0;",
          "  for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var dd=dayDiff(a.start_date);if(dd!=null&&dd>=0)soon++;var p=a.policy_acknowledgement;if(Array.isArray(p)&&p.length>=5)pol++;}",
          "  var html='<div class=\"wrap\">';",
          "  html+=headerBlock(I.uplus,ctx.title||'Employee Onboarding',brief(['<b>'+fmtInt(rows.length)+'</b> employees onboarded',soon?'<b>'+fmtInt(soon)+'</b> starting soon':'','<b>'+fmtInt(pol)+'</b> fully policy-signed']),'New onboarding');",
          "  html+='<div class=\"kpis\">'+kpi('Onboarded',rows.length,soon?'<b>'+fmtInt(soon)+'</b> starting soon':'No upcoming start dates',{hero:true,spark:spark(weekly(rows))})+kpi('Starting soon',soon,'start date today or later')+kpi('Policies signed',pol,'all five policies acknowledged')+'</div>';",
          "  var rec='';",
    ...
    // ── 4. Leave Request ──────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "  if(up.length){var lastKey=null,shown=0;for(var k=0;k<up.length&&shown<8;k++){var it=up[k];var key=String(it.a.start_date||'');if(key!==lastKey){if(lastKey!==null)schedHtml+='</div>';schedHtml+='<div class=\"day\"><div class=\"day-h\">'+h(dayLabel(it.dd,it.a.start_date))+'</div>';lastKey=key;}var days=num(it.a.total_days);schedHtml+='<div class=\"sch\"><span class=\"sch-t\">'+h(days?fmtInt(days)+'d':'\\u2014')+'</span><div class=\"row-main\"><div class=\"row-title\">'+h(it.a.employee_name||'Employee')+'</div><div class=\"row-sub\">'+h(labelFor(typeMap,it.a.leave_type))+(it.a.end_date?' \\u00b7 until '+h(fmtDate(it.a.end_date)):'')+'</div></div><div class=\"row-r\"><span class=\"chip\">'+h(ago(it.a.start_date))+'</span></div></div>';shown++;}schedHtml+='</div>';}",
          "  else if(rows.length){mainTitle='Latest requests';for(var r2=0;r2<Math.min(6,rows.length);r2++){var rb=rows[r2].answers||{};schedHtml+=rowItem(h(rb.employee_name||'Employee'),h(labelFor(typeMap,rb.leave_type))+' \\u00b7 '+h(fmtDate(rb.start_date)),'<span class=\"chip\">'+h(fmtInt(num(rb.total_days)))+'d</span>');}}",
    ...
    // ── 7. Expense Claim ──────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "  var now=new Date(),moT=0,moN=0;",
          "  for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var d=pd(a.expense_date)||pd(rows[i].submittedAt);if(d&&d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear()){moT+=num(a.amount);moN++;}}",
          "  var html='<div class=\"wrap\">';",
          "  html+=headerBlock(I.receipt,ctx.title||'Expense Claim',brief(['<b>'+fmtInt(rows.length)+'</b> claims submitted','<b>'+mnyC(total)+'</b> claimed in total',moN?'<b>'+mnyC(moT)+'</b> this month':'']),'New claim');",
          "  html+='<div class=\"kpis\">'+kpi('Claims',rows.length,total?'<b>'+mnyC(total)+'</b> claimed in total':'No claims yet',{hero:true,spark:spark(weekly(rows))})+kpi('Total claimed',mnyC(total),'across all claims')+kpi('This month',mnyC(moT),'<b>'+fmtInt(moN)+'</b> claims dated this month')+'</div>';",
          "  var rec='';",
          "  for(var r=0;r<Math.min(6,rows.length);r++){var a2=rows[r].answers||{};rec+=rowItem(h(a2.description||a2.employee_name||'Expense'),h(a2.employee_name||'')+(a2.employee_name?' \\u00b7 ':'')+h(labelFor(catMap,a2.category))+' \\u00b7 '+h(fmtDate(a2.expense_date||rows[r].submittedAt)),'<span class=\"amt\">'+mny(a2.amount)+'</span>');}",
    ...
    // ── 8. Training Request ───────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "  var rec='';",
          "  for(var r=0;r<Math.min(6,rows.length);r++){var a2=rows[r].answers||{};var c2=num(a2.cost);rec+=rowItem(h(a2.course_name||'Training'),h(a2.employee_name||'')+(a2.employee_name?' \\u00b7 ':'')+h(labelFor(typeMap,a2.training_type))+' \\u00b7 '+h(fmtDate(a2.start_date||rows[r].submittedAt)),c2>0?'<span class=\"amt\">'+mnyC(c2)+'</span>':'<span class=\"chip\">'+h(ago(rows[r].submittedAt))+'</span>');}",
