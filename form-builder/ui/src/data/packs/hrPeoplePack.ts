...
// ── Section-screen kit (shared by every form's customScreen below) ──────────
  ...
  forms: [
    // ── 1. Job Application ────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        css: SECTION_CSS,
        js: SECTION_KIT_JS + '\n' + [
          "I.briefcase=icoSvg('<rect x=\"3.5\" y=\"7\" width=\"17\" height=\"13\" rx=\"2\"/><path d=\"M9 7V5.5A1.5 1.5 0 0 1 10.5 4h3A1.5 1.5 0 0 1 15 5.5V7M3.5 12.5h17\"/>');",
          "async function main(){",
          "  var root=document.getElementById('app');",
          "  var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "  var rows=await recs(500);",
          "  var posMap=optionMap(ctx.fields,'position');",
          "  var now=new Date(),mo=0;",
          "  for(var i=0;i<rows.length;i++){var d=pd(rows[i].submittedAt);if(d&&d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear())mo++;}",
          "  var posN=Object.keys(countBy(rows,'position')).length;",
          "  var html='<div class=\"wrap\">';",
          "  html+=headerBlock(I.briefcase,ctx.title||'Job Application',brief(['<b>'+fmtInt(rows.length)+'</b> applicants in the pipeline','<b>'+fmtInt(mo)+'</b> arrived this month',posN?'<b>'+fmtInt(posN)+'</b> positions in play':'']),'New application');",
          "  html+='<div class=\"kpis\">'+kpi('Applicants',rows.length,mo?'<b>'+fmtInt(mo)+'</b> new this month':'Awaiting the first application',{hero:true,spark:spark(weekly(rows))})+kpi('This month',mo,'applications received')+kpi('Positions',posN,'roles attracting candidates')+'</div>';",
          "  var rec='';",
          "  for(var r=0;r<Math.min(6,rows.length);r++){var a=rows[r].answers||{};var nm=((a.first_name||'')+' '+(a.last_name||'')).trim()||'Candidate';var right=num(a.salary_expectations)>0?'<span class=\"amt\">'+mnyC(a.salary_expectations)+'</span><span class=\"chip\">expected salary</span>':'<span class=\"chip\">'+h(ago(rows[r].submittedAt))+'</span>';rec+=rowItem(h(nm),h(labelFor(posMap,a.position))+' \\u00b7 '+h(fmtDate(rows[r].submittedAt)),right);}",
          "  var recBody=rec?'<div class=\"rows\">'+rec+'</div>':emptyBlock(I.user,'New applicants land here as they apply.','+ New application');",
          "  html+=sec('Pipeline intake')+'<div class=\"grid g21\">'+panel('Applications by position',breakdown(rows,'position',posMap)||emptyBlock(I.chart,'The position mix appears once applications arrive.','+ New application'))+panel('Recent applicants',recBody)+'</div>';",
    ...
    // ── 2. Interview Scorecard ────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        css: SECTION_CSS,
        js: SECTION_KIT_JS + '\n' + [
          "I.star=icoSvg('<path d=\"M12 4l2.3 5.45 5.9.5-4.5 3.85 1.35 5.75-5.05-3.05-5.05 3.05 1.35-5.75-4.5-3.85 5.9-.5z\"/>');",
          "async function main(){",
          "  var root=document.getElementById('app');",
          "  var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "  var rows=await recs(500);",
          "  var recMap=optionMap(ctx.fields,'overall_recommendation');",
          "  var typeMap=optionMap(ctx.fields,'interview_type');",
          "  var hire=0,sSum=0,sN=0;",
          "  for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var rc=a.overall_recommendation;if(rc==='hire'||rc==='strong_hire')hire++;var s=num(a.overall_score);if(!s){var ds=[a.technical_skills,a.communication,a.culture_fit,a.problem_solving],t=0,n=0;for(var j=0;j<ds.length;j++){var v=num(ds[j]);if(v>0){t+=v;n++;}}if(n)s=t/n;}if(s>0){sSum+=s;sN++;}}",
          "  var pct=rows.length?Math.round(hire/rows.length*100):0;var avg=sN?sSum/sN:0;",
          "  var html='<div class=\"wrap\">';",
          "  html+=headerBlock(I.star,ctx.title||'Interview Scorecard',brief(['<b>'+fmtInt(rows.length)+'</b> interviews scored','<b>'+pct+'%</b> recommend hire',avg?'avg score <b>'+avg.toFixed(1)+'</b> / 5':'']),'New scorecard');",
          "  html+='<div class=\"kpis\">'+kpi('Interviews scored',rows.length,'',{hero:true,spark:spark(weekly(rows))})+kpi('Recommend hire',pct+'%','<b>'+fmtInt(hire)+'</b> of '+fmtInt(rows.length)+' candidates')+kpi('Avg score',avg?avg.toFixed(1):'\\u2014','out of 5, all dimensions')+'</div>';",
          "  var rec='';",
          "  for(var r=0;r<Math.min(6,rows.length);r++){var a2=rows[r].answers||{};var rc2=a2.overall_recommendation;var kind=(rc2==='strong_hire'||rc2==='hire')?'good':rc2==='no_hire'?'warn':rc2==='strong_no_hire'?'bad':'neutral';var sc=num(a2.overall_score);rec+=rowItem(h(a2.interviewer_name||'Interviewer'),h(labelFor(typeMap,a2.interview_type))+' \\u00b7 '+h(fmtDate(a2.interview_date||rows[r].submittedAt)),pill(labelFor(recMap,rc2),kind)+(sc?'<span class=\"chip\">'+sc.toFixed(1)+' / 5</span>':''));}",
          "  var recBody=rec?'<div class=\"rows\">'+rec+'</div>':emptyBlock(I.check,'Completed scorecards appear here.','+ New scorecard');",
          "  html+=sec('Hiring signal')+'<div class=\"grid g21\">'+panel('Recommendations',breakdown(rows,'overall_recommendation',recMap)||emptyBlock(I.chart,'The recommendation mix appears after the first interview.','+ New scorecard'))+panel('Recent interviews',recBody)+'</div>';",
          "  html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 3. Employee Onboarding ────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        css: SECTION_CSS,
        js: SECTION_KIT_JS + '\n' + [
          "I.uplus=icoSvg('<circle cx=\"10\" cy=\"8\" r=\"3.5\"/><path d=\"M3 20.5c.7-3.4 3.7-5 7-5 1.5 0 2.9.3 4 .9\"/><path d=\"M18 14.5v6M15 17.5h6\"/>');",
          "async function main(){",
          "  var root=document.getElementById('app');",
          "  var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "  var rows=await recs(500);",
          "  var deptMap=optionMap(ctx.fields,'department');",
          "  var soon=0,pol=0,polT=0;",
          "  for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var dd=dayDiff(a.start_date);if(dd!=null&&dd>=0)soon++;var p=a.policy_acknowledgement;if(Array.isArray(p)){polT+=p.length;if(p.length>=5)pol++;}}",
          "  var polPct=rows.length?Math.round(polT/(rows.length*5)*100):0;",
          "  var html='<div class=\"wrap\">';",
          "  html+=headerBlock(I.uplus,ctx.title||'Employee Onboarding',brief(['<b>'+fmtInt(rows.length)+'</b> employees onboarded',soon?'<b>'+fmtInt(soon)+'</b> starting soon':'',rows.length?'<b>'+polPct+'%</b> of policies signed':'']),'New onboarding');",
          "  html+='<div class=\"kpis\">'+kpi('Onboarded',rows.length,soon?'<b>'+fmtInt(soon)+'</b> starting soon':'No upcoming start dates',{hero:true,spark:spark(weekly(rows))})+kpi('Starting soon',soon,'start date today or later')+kpi('Policies signed',rows.length?polPct+'%':'\\u2014','<b>'+fmtInt(pol)+'</b> of '+fmtInt(rows.length)+' fully signed')+'</div>';",
          "  var rec='';",
          "  for(var r=0;r<Math.min(6,rows.length);r++){var a2=rows[r].answers||{};rec+=rowItem(h(a2.employee_name||'Employee'),h(labelFor(deptMap,a2.department))+' \\u00b7 starts '+h(fmtDate(a2.start_date)),'<span class=\"chip\">'+h(ago(a2.start_date)||ago(rows[r].submittedAt))+'</span>');}",
          "  var recBody=rec?'<div class=\"rows\">'+rec+'</div>':emptyBlock(I.user,'New starters appear here.','+ New onboarding');",
          "  html+=sec('New starters')+'<div class=\"grid g21\">'+panel('Onboarding by department',breakdown(rows,'department',deptMap)||emptyBlock(I.chart,'The department mix appears once employees are onboarded.','+ New onboarding'))+panel('Recent starters',recBody)+'</div>';",
          "  html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 4. Leave Request ──────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        css: SECTION_CSS,
        js: SECTION_KIT_JS + '\n' + [
          "I.leaf=icoSvg('<path d=\"M5 19c9.5 1.5 14.5-3.5 15-13-9.5-.5-14.5 3.5-15 13z\"/><path d=\"M5 19c2-5.5 5.5-9 10-11\"/>');",
          "function dayLabel(dd,ds){if(dd===0)return 'Today';if(dd===1)return 'Tomorrow';var d=pd(ds);return d?d.toLocaleDateString(undefined,{weekday:'short',month:'short',day:'numeric'}):'Scheduled';}",
          "function spanDays(s,e){var a=pd(s),b=pd(e);return (a&&b&&b.getTime()>=a.getTime())?Math.round((b.getTime()-a.getTime())/86400000)+1:null;}",
          "async function main(){",
          "  var root=document.getElementById('app');",
          "  var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "  var rows=await recs(500);",
          "  var typeMap=optionMap(ctx.fields,'leave_type');",
          "  var totDays=sumBy(rows,'total_days');",
          "  var up=[];for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var dd=dayDiff(a.start_date);if(dd!=null&&dd>=0)up.push({a:a,dd:dd});}",
          "  up.sort(function(x,y){return x.dd-y.dd;});",
          "  var daysV=totDays%1?totDays.toFixed(1):totDays;",
          "  var html='<div class=\"wrap\">';",
          "  html+=headerBlock(I.leaf,ctx.title||'Leave Request',brief(['<b>'+fmtInt(rows.length)+'</b> leave requests','<b>'+fmtInt(totDays)+'</b> days requested',up.length?'<b>'+fmtInt(up.length)+'</b> upcoming':'']),'New leave request');",
          "  html+='<div class=\"kpis\">'+kpi('Leave requests',rows.length,totDays?'<b>'+fmtInt(totDays)+'</b> days requested in total':'No requests yet',{hero:true,spark:spark(weekly(rows))})+kpi('Days requested',daysV,'across all requests')+kpi('Upcoming',up.length,'leave starting today or later')+'</div>';",
          "  var schedHtml='';var mainTitle='Upcoming leave';",
          "  if(up.length){var lastKey=null,shown=0;for(var k=0;k<up.length&&shown<8;k++){var it=up[k];var key=String(it.a.start_date||'');if(key!==lastKey){if(lastKey!==null)schedHtml+='</div>';schedHtml+='<div class=\"day\"><div class=\"day-h\">'+h(dayLabel(it.dd,it.a.start_date))+'</div>';lastKey=key;}var days=spanDays(it.a.start_date,it.a.end_date)||num(it.a.total_days);schedHtml+='<div class=\"sch\"><span class=\"sch-t\">'+h(days?fmtInt(days)+'d':'\\u2014')+'</span><div class=\"row-main\"><div class=\"row-title\">'+h(it.a.employee_name||'Employee')+'</div><div class=\"row-sub\">'+h(labelFor(typeMap,it.a.leave_type))+(it.a.end_date?' \\u00b7 until '+h(fmtDate(it.a.end_date)):'')+'</div></div><div class=\"row-r\"><span class=\"chip\">'+h(ago(it.a.start_date))+'</span></div></div>';shown++;}schedHtml+='</div>';}",
          "  else if(rows.length){mainTitle='Latest requests';for(var r2=0;r2<Math.min(6,rows.length);r2++){var rb=rows[r2].answers||{};schedHtml+=rowItem(h(rb.employee_name||'Employee'),h(labelFor(typeMap,rb.leave_type))+' \\u00b7 '+h(fmtDate(rb.start_date)),'<span class=\"chip\">'+h(fmtInt(spanDays(rb.start_date,rb.end_date)||num(rb.total_days)))+'d</span>');}}",
          "  if(!schedHtml)schedHtml=emptyBlock(I.cal,'Approved time away will appear here by start date.','+ New leave request');",
          "  var bd=breakdown(rows,'leave_type',typeMap)||emptyBlock(I.chart,'The leave-type mix appears once requests come in.','+ New leave request');",
          "  html+=sec('Time away')+'<div class=\"grid g21\">'+panel(mainTitle,schedHtml)+panel('By leave type',bd)+'</div>';",
          "  html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 5. Performance Review ─────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        css: SECTION_CSS,
        js: SECTION_KIT_JS + '\n' + [
          "I.trend=icoSvg('<path d=\"M3.5 17.5l6-6 4 4 7-7.5\"/><path d=\"M15.5 8h5v5\"/>');",
          "async function main(){",
          "  var root=document.getElementById('app');",
          "  var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "  var rows=await recs(500);",
          "  var gmMap=optionMap(ctx.fields,'goals_met');var prMap=optionMap(ctx.fields,'review_period');",
          "  var sSum=0,sN=0,metN=0;",
          "  for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var s=num(a.overall_rating);if(!s){var ds=[a.job_knowledge,a.quality_of_work,a.productivity,a.teamwork,a.initiative],t=0,n=0;for(var j=0;j<ds.length;j++){var v=num(ds[j]);if(v>0){t+=v;n++;}}if(n)s=t/n;}if(s>0){sSum+=s;sN++;}if(a.goals_met==='met'||a.goals_met==='exceeded')metN++;}",
          "  var avg=sN?sSum/sN:0;var metPct=rows.length?Math.round(metN/rows.length*100):0;",
          "  var html='<div class=\"wrap\">';",
          "  html+=headerBlock(I.trend,ctx.title||'Performance Review',brief(['<b>'+fmtInt(rows.length)+'</b> reviews completed',avg?'avg rating <b>'+avg.toFixed(1)+'</b> / 5':'',rows.length?'<b>'+metPct+'%</b> met or exceeded goals':'']),'New review');",
          "  html+='<div class=\"kpis\">'+kpi('Reviews',rows.length,'',{hero:true,spark:spark(weekly(rows))})+kpi('Avg rating',avg?avg.toFixed(1):'\\u2014','out of 5, all dimensions')+kpi('Goals met',metPct+'%','<b>'+fmtInt(metN)+'</b> met or exceeded goals')+'</div>';",
          "  var rec='';",
          "  for(var r=0;r<Math.min(6,rows.length);r++){var a2=rows[r].answers||{};var gm=a2.goals_met;var kind=gm==='exceeded'?'good':gm==='met'?'accent':gm==='partially_met'?'warn':gm==='not_met'?'bad':'neutral';var sc=num(a2.overall_rating);rec+=rowItem(h(a2.reviewer_name||'Reviewer'),h(labelFor(prMap,a2.review_period))+' \\u00b7 '+h(fmtDate(a2.review_date||rows[r].submittedAt)),pill(labelFor(gmMap,gm),kind)+(sc?'<span class=\"chip\">'+sc.toFixed(1)+' / 5</span>':''));}",
          "  var recBody=rec?'<div class=\"rows\">'+rec+'</div>':emptyBlock(I.check,'Completed reviews appear here.','+ New review');",
          "  html+=sec('Review cycle')+'<div class=\"grid g21\">'+panel('Goals met',breakdown(rows,'goals_met',gmMap)||emptyBlock(I.chart,'The goals-met mix appears after the first review.','+ New review'))+panel('Recent reviews',recBody)+'</div>';",
          "  html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 6. Exit Interview ─────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        css: SECTION_CSS,
        js: SECTION_KIT_JS + '\n' + [
          "I.exit=icoSvg('<path d=\"M10 4H6.5A2.5 2.5 0 0 0 4 6.5v11A2.5 2.5 0 0 0 6.5 20H10\"/><path d=\"M15 8l4 4-4 4M19 12H9\"/>');",
          "async function main(){",
          "  var root=document.getElementById('app');",
          "  var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "  var rows=await recs(500);",
          "  var reasonMap=optionMap(ctx.fields,'reason_for_leaving');var deptMap=optionMap(ctx.fields,'department');",
          "  var satSum=0,satN=0,retY=0;",
          "  for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var s=num(a.overall_satisfaction);if(s>0){satSum+=s;satN++;}if(a.would_return==='yes')retY++;}",
          "  var avg=satN?satSum/satN:0;var retPct=rows.length?Math.round(retY/rows.length*100):0;",
          "  var html='<div class=\"wrap\">';",
          "  html+=headerBlock(I.exit,ctx.title||'Exit Interview',brief(['<b>'+fmtInt(rows.length)+'</b> exit interviews',avg?'avg satisfaction <b>'+avg.toFixed(1)+'</b> / 5':'',rows.length?'<b>'+retPct+'%</b> would return':'']),'New exit interview');",
          "  html+='<div class=\"kpis\">'+kpi('Exit interviews',rows.length,'',{hero:true,spark:spark(weekly(rows))})+kpi('Avg satisfaction',avg?avg.toFixed(1):'\\u2014','out of 5 from departing staff')+kpi('Would return',retPct+'%','<b>'+fmtInt(retY)+'</b> said yes')+'</div>';",
          "  var rec='';",
          "  for(var r=0;r<Math.min(6,rows.length);r++){var a2=rows[r].answers||{};var wr=a2.would_return;var kind=wr==='yes'?'good':wr==='maybe'?'warn':wr==='no'?'bad':'neutral';var pl=wr==='yes'?'would return':wr==='maybe'?'might return':wr==='no'?'would not return':'\\u2014';var s2=num(a2.overall_satisfaction);rec+=rowItem(h(a2.employee_name||'Employee'),h(labelFor(deptMap,a2.department))+' \\u00b7 last day '+h(fmtDate(a2.last_day)),pill(pl,kind)+(s2?'<span class=\"chip\">'+s2.toFixed(0)+' / 5</span>':''));}",
          "  var recBody=rec?'<div class=\"rows\">'+rec+'</div>':emptyBlock(I.user,'Departing employees appear here.','+ New exit interview');",
          "  html+=sec('Departures')+'<div class=\"grid g21\">'+panel('Reasons for leaving',breakdown(rows,'reason_for_leaving',reasonMap)||emptyBlock(I.chart,'The reason mix appears after the first exit interview.','+ New exit interview'))+panel('Recent departures',recBody)+'</div>';",
          "  html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 7. Expense Claim ──────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        css: SECTION_CSS,
        js: SECTION_KIT_JS + '\n' + [
          "I.receipt=icoSvg('<path d=\"M6.5 3.5h11V20.5l-1.85-1.3-1.83 1.3-1.82-1.3-1.82 1.3-1.83-1.3L6.5 20.5z\"/><path d=\"M9.5 8.5h5M9.5 12h5\"/>');",
          "async function main(){",
          "  var root=document.getElementById('app');",
          "  var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "  var rows=await recs(500);",
          "  var catMap=optionMap(ctx.fields,'category');",
          "  var total=sumBy(rows,'amount');",
          "  var moT=0,moN=0;",
          "  for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var dd=dayDiff(a.expense_date||rows[i].submittedAt);if(dd!=null&&dd<=0&&dd>-30){moT+=num(a.amount);moN++;}}",
          "  var html='<div class=\"wrap\">';",
          "  html+=headerBlock(I.receipt,ctx.title||'Expense Claim',brief(['<b>'+fmtInt(rows.length)+'</b> claims submitted','<b>'+mnyC(total)+'</b> claimed in total',moN?'<b>'+mnyC(moT)+'</b> in the last 30 days':'']),'New claim');",
          "  html+='<div class=\"kpis\">'+kpi('Claims',rows.length,total?'<b>'+mnyC(total)+'</b> claimed in total':'No claims yet',{hero:true,spark:spark(weekly(rows))})+kpi('Total claimed',mnyC(total),'across all claims')+kpi('Last 30 days',mnyC(moT),'<b>'+fmtInt(moN)+'</b> claims in the last 30 days')+'</div>';",
          "  var rec='';",
          "  for(var r=0;r<Math.min(6,rows.length);r++){var a2=rows[r].answers||{};rec+=rowItem(h(a2.description||a2.employee_name||'Expense'),h(a2.employee_name||'')+(a2.employee_name?' \\u00b7 ':'')+h(labelFor(catMap,a2.category)),'<span class=\"amt\">'+mny(a2.amount)+'</span><span class=\"chip\">'+h(fmtDate(a2.expense_date||rows[r].submittedAt))+'</span>');}",
          "  var recBody=rec?'<div class=\"rows\">'+rec+'</div>':emptyBlock(I.doc,'Submitted claims appear here with amounts.','+ New claim');",
          "  html+=sec('Spend')+'<div class=\"grid g21\">'+panel('Claims by category',breakdown(rows,'category',catMap)||emptyBlock(I.chart,'The category mix appears once claims are submitted.','+ New claim'))+panel('Recent claims',recBody)+'</div>';",
          "  html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 8. Training Request ───────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        css: SECTION_CSS,
        js: SECTION_KIT_JS + '\n' + [
          "I.grad=icoSvg('<path d=\"M12 4 2.5 8.5 12 13l9.5-4.5z\"/><path d=\"M6.2 10.8V15c0 1.6 2.6 2.9 5.8 2.9s5.8-1.3 5.8-2.9v-4.2\"/><path d=\"M21.5 8.5v5\"/>');",
          "async function main(){",
          "  var root=document.getElementById('app');",
          "  var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "  var rows=await recs(500);",
          "  var typeMap=optionMap(ctx.fields,'training_type');",
          "  var cost=sumBy(rows,'cost');var upN=0;",
          "  for(var i=0;i<rows.length;i++){var dd=dayDiff((rows[i].answers||{}).start_date);if(dd!=null&&dd>=0)upN++;}",
          "  var html='<div class=\"wrap\">';",
          "  html+=headerBlock(I.grad,ctx.title||'Training Request',brief(['<b>'+fmtInt(rows.length)+'</b> training requests','<b>'+mnyC(cost)+'</b> estimated cost',upN?'<b>'+fmtInt(upN)+'</b> starting soon':'']),'New training request');",
          "  html+='<div class=\"kpis\">'+kpi('Requests',rows.length,cost?'<b>'+mnyC(cost)+'</b> in estimated cost':'No requests yet',{hero:true,spark:spark(weekly(rows))})+kpi('Estimated cost',mnyC(cost),'across all requests')+kpi('Starting soon',upN,'programs starting today or later')+'</div>';",
          "  var rec='';",
          "  for(var r=0;r<Math.min(6,rows.length);r++){var a2=rows[r].answers||{};var c2=num(a2.cost);rec+=rowItem(h(a2.employee_name||'Employee'),h(a2.course_name||'Training')+' \\u00b7 '+h(labelFor(typeMap,a2.training_type)),(c2>0?'<span class=\"amt\">'+mnyC(c2)+'</span>':'')+'<span class=\"chip\">'+h(fmtDate(a2.start_date||rows[r].submittedAt))+'</span>');}",
          "  var recBody=rec?'<div class=\"rows\">'+rec+'</div>':emptyBlock(I.doc,'Training and development requests appear here.','+ New training request');",
          "  html+=sec('Development')+'<div class=\"grid g21\">'+panel('Requests by type',breakdown(rows,'training_type',typeMap)||emptyBlock(I.chart,'The training-type mix appears once requests come in.','+ New training request'))+panel('Recent requests',recBody)+'</div>';",
          "  html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
  ...
  apps: [
    {
      ...
      customScreen: {
        ...
        css: [
          ...
          '@media(max-width:960px){.kpis{grid-template-columns:1fr 1fr;}.g21,.g12,.g11,.g111{grid-template-columns:1fr;}}',
          '@media(max-width:600px){.wrap{padding:20px 14px 48px;}.kpi-hero{grid-column:1/-1;}.kpis .kpi:last-child:nth-child(even){grid-column:1/-1;}.hdr-r{width:100%;flex-wrap:wrap;}.hdr-r .btn{flex:1;justify-content:center;}.bar-row{grid-template-columns:minmax(76px,110px) 1fr 40px;}.title{font-size:21px;}}',
          '.funnel .bar-row{grid-template-columns:minmax(96px,160px) 1fr 46px;padding:7px 0;}',
          '.fun-note{margin-top:10px;font-size:12px;color:var(--fl-faint);}',
        ].join('\n'),
        js: [
          "var FL=window.FormLogic;",
          "var RM=false;try{RM=window.matchMedia('(prefers-reduced-motion: reduce)').matches;}catch(e){}",
          ...
          "function fatal(root,msg){root.innerHTML='<div class=\"wrap\"><div class=\"load\">'+h(msg)+'</div></div>';}",
          "I.star=icoSvg('<path d=\"M12 4l2.3 5.45 5.9.5-4.5 3.85 1.35 5.75-5.05-3.05-5.05 3.05 1.35-5.75-4.5-3.85 5.9-.5z\"/>');",
          "I.book=icoSvg('<path d=\"M4 19.5A2.5 2.5 0 0 1 6.5 17H20\"/><path d=\"M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z\"/>');",
          "function spanDays(s,e){var a=pd(s),b=pd(e);return (a&&b&&b.getTime()>=a.getTime())?Math.round((b.getTime()-a.getTime())/86400000)+1:null;}",
          "async function main(){",
          "  var root=document.getElementById('app');",
          ...
          "  var fCard=findForm(ctx,'Interview Scorecards');",
          ...
          "  var res=await Promise.all([recs(fApp),recs(fCard),recs(fOnb),recs(fLeave),recs(fExp)]);",
          "  var apps=res[0],cards=res[1],onb=res[2],leave=res[3],exp=res[4];",
          "  var posMap=optionMap(fApp,'position'),refMap=optionMap(fApp,'referral_source'),typeMap=optionMap(fLeave,'leave_type');",
          "  var expTotal=sumBy(exp,'amount'),leaveDays=sumBy(leave,'total_days');",
          "  var startingSoon=0;for(var i=0;i<onb.length;i++){var sd=dayDiff((onb[i].answers||{}).start_date);if(sd!=null&&sd>=0)startingSoon++;}",
          "  function addIds(set,v){if(v==null||v==='')return;if(Array.isArray(v)){for(var j=0;j<v.length;j++){if(v[j]!=null&&v[j]!=='')set[v[j]]=1;}}else{set[v]=1;}}",
          "  var ivSet={},recSet={};",
          "  for(var c=0;c<cards.length;c++){var ca=cards[c].answers||{};addIds(ivSet,ca.candidate_record);var rc=ca.overall_recommendation;if(rc==='hire'||rc==='strong_hire')addIds(recSet,ca.candidate_record);}",
          "  var interviewed=Object.keys(ivSet).length,recommended=Object.keys(recSet).length;",
          "  var wk=weekly(apps),newWk=wk.length?wk[wk.length-1]:0;",
          "  var appNav=fApp?'form/'+fApp.formId+'/responses':'';",
          "  var html='<div class=\"wrap\">';",
          "  html+=headerBlock(I.users,ctx.appName||'People Hub',brief(['<b>'+fmtInt(apps.length)+'</b> applicants','<b>'+fmtInt(cards.length)+'</b> interviews scored','<b>'+fmtInt(onb.length)+'</b> onboarding','<b>'+mnyC(expTotal)+'</b> in expense claims']),[{nav:fApp?fApp.formId:'',label:'New application',icon:I.plus},{nav:fCard?fCard.formId:'',label:'Score interview',icon:I.star,ghost:true}]);",
          "  html+='<div class=\"kpis\">'+kpi('Candidates in pipeline',apps.length?apps.length:'\\u2014',apps.length?'<b>'+fmtInt(newWk)+'</b> new this week':'No data yet \\u00b7 add your first application',{hero:true,spark:spark(wk)})+kpi('Onboarding',onb.length,onb.length?(startingSoon?'<b>'+fmtInt(startingSoon)+'</b> starting soon':'None starting soon'):'No records yet')+kpi('Leave days requested',fmtInt(leaveDays),'<b>'+fmtInt(leave.length)+'</b> requests')+kpi('Expenses',mnyC(expTotal),'<b>'+fmtInt(exp.length)+'</b> claims')+'</div>';",
          "  var fmax=Math.max(apps.length,interviewed,recommended,onb.length);",
          "  var funnel;",
          "  if(fmax>0){funnel='<div class=\"funnel\">'+barRow('Applications',apps.length,fmax)+barRow('Interviewed',interviewed,fmax,'color-mix(in srgb, var(--fl-accent) 72%, transparent)')+barRow('Recommended',recommended,fmax,'color-mix(in srgb, var(--fl-accent) 46%, transparent)')+barRow('Onboarded',onb.length,fmax,'var(--fl-good)')+'</div>';if(apps.length>0&&onb.length<=apps.length){funnel+='<div class=\"fun-note\">'+Math.round(onb.length/apps.length*100)+'% of applications reach onboarding</div>';}else if(onb.length>0){funnel+='<div class=\"fun-note\"><b>'+fmtInt(onb.length)+'</b> onboarded</div>';}}",
          "  else{funnel=emptyBlock(I.users,'Your hiring funnel builds here as applications arrive.',fApp?fApp.formId:'','+ New application');}",
          "  html+=sec('Hiring pipeline')+'<div class=\"grid g21\">'+panel('Pipeline stages',funnel,appNav,'View applications')+panel('Applications by position',breakdown(apps,'position',posMap)||emptyBlock(I.chart,'Position mix appears once applications arrive.',fApp?fApp.formId:'','+ New application'),appNav)+'</div>';",
          "  var up=[];for(var l=0;l<leave.length;l++){var dd=dayDiff((leave[l].answers||{}).start_date);if(dd!=null&&dd>=0)up.push(leave[l]);}",
          "  up.sort(function(x,y){var dx=pd((x.answers||{}).start_date),dy=pd((y.answers||{}).start_date);return (dx?dx.getTime():0)-(dy?dy.getTime():0);});",
          "  var lvTitle=up.length?'Upcoming leave':'Recent leave requests',lvList=up.length?up:leave;",
          "  var lvHtml='';",
          "  for(var v=0;v<Math.min(5,lvList.length);v++){var lr=lvList[v].answers||{};var lt=lr.leave_type;var kind=lt==='sick'?'warn':(lt==='annual'?'accent':'neutral');var when=ago(lr.start_date);var ps=pd(lr.start_date),pe=pd(lr.end_date);var rng=h(fmtDate(lr.start_date));if(ps&&pe&&pe.getTime()>=ps.getTime()){rng+=' \\u2013 '+h(fmtDate(lr.end_date));}lvHtml+=rowItem(h(lr.employee_name||'Employee'),rng+(when?' \\u00b7 '+h(when):''),pill(labelFor(typeMap,lt),kind)+'<span class=\"chip\">'+fmtInt(spanDays(lr.start_date,lr.end_date)||num(lr.total_days))+'d</span>');}",
          "  var lvBody=lvHtml?'<div class=\"rows\">'+lvHtml+'</div>':emptyBlock(I.cal,'Leave requests will appear here.',fLeave?fLeave.formId:'','+ Log leave');",
          "  var raHtml='';",
          "  for(var r=0;r<Math.min(5,apps.length);r++){var aa=apps[r].answers||{};var nm=((aa.first_name||'')+' '+(aa.last_name||'')).trim()||'Candidate';var sub=h(labelFor(posMap,aa.position));if(aa.referral_source){sub+=' \\u00b7 '+h(labelFor(refMap,aa.referral_source));}raHtml+=rowItem(h(nm),sub,'<span class=\"chip\">'+h(fmtDate(apps[r].submittedAt))+'</span>');}",
          "  var raBody=raHtml?'<div class=\"rows\">'+raHtml+'</div>':emptyBlock(I.doc,'New applications land here.',fApp?fApp.formId:'','+ New application');",
          "  html+=sec('PTO & candidates')+'<div class=\"grid g11\">'+panel(lvTitle,lvBody,fLeave?'form/'+fLeave.formId+'/responses':'')+panel('Recent applications',raBody,appNav)+'</div>';",
          "  html+=sec('Quick actions')+acts([{nav:fApp?fApp.formId:'',label:'New application',icon:I.doc},{nav:fCard?fCard.formId:'',label:'Score interview',icon:I.star},{nav:fOnb?fOnb.formId:'',label:'Onboard employee',icon:I.user},{nav:fLeave?fLeave.formId:'',label:'Log leave',icon:I.cal},{nav:fExp?fExp.formId:'',label:'Submit expense',icon:I.money},{nav:fTrain?fTrain.formId:'',label:'Request training',icon:I.book}]);",
          "  html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
