    ...
    // ── 1. New Client Onboarding ─────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};if(String(a.accredited_investor)==='Yes')accredited++;totalNw+=num(a.net_worth);var dd=dayDiff(rows[i].submittedAt);if(dd!=null&&dd>-30)recent30++;}",
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};var nm=(String(an.first_name||'')+' '+String(an.last_name||'')).trim()||'Client';var right=(String(an.accredited_investor)==='Yes'?pill('Accredited','good'):pill('Retail','neutral'))+'<span class=\"chip\">'+h(ago(r.submittedAt))+'</span>';list+=rowItem(h(nm),h(labelFor(objMap,an.investment_objectives))+' \\u00b7 '+h(fmtDate(r.submittedAt)),right);}",
    ...
    // ── 2. Risk Tolerance Questionnaire ──────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};if(String(a.reg_bi_check)==='Review needed')flagged++;var s=num(a.risk_profile_score);if(s>0){scoreSum+=s;scoreN++;}}",
          "var avg=scoreN?Math.round(scoreSum/scoreN):0;",
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};var flag=String(an.reg_bi_check)==='Review needed';var right=(flag?pill('Review needed','bad'):pill('Suitable','good'))+'<span class=\"chip\">score '+fmtInt(num(an.risk_profile_score))+'</span>';list+=rowItem(h(labelFor(ptMap,an.portfolio_type))+' portfolio',h(labelFor(exMap,an.investment_experience))+' \\u00b7 '+h(fmtDate(r.submittedAt)),right);}",
          "var glyph=icoSvg('<path d=\"M4.5 19a8.5 8.5 0 1 1 15 0\"/><path d=\"M12 13l3.5-3.5\"/><circle cx=\"12\" cy=\"13\" r=\"1.4\"/>');",
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(glyph,ctx.title||'Risk Tolerance Questionnaire',brief(['<b>'+fmtInt(rows.length)+'</b> assessments',flagged?'<b>'+fmtInt(flagged)+'</b> flagged for review':'none flagged','avg score <b>'+fmtInt(avg)+'</b>']),'New assessment');",
          "html+='<div class=\"kpis\">'+kpi('Assessments',rows.length,'',{hero:true,spark:spark(weekly(rows))})+kpi('Flagged',flagged,flagged?pill('Reg BI review','bad'):pill('All suitable','good'))+kpi('Avg score',avg,'suitability score, 1\\u2013100')+'</div>';",
    ...
    // ── 4. Form CRS & Relationship Summary ───────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};list+=rowItem(h(String(an.advisor_name||'').trim()||'Acknowledgement'),'Acknowledged '+h(fmtDate(an.acknowledgement_date||r.submittedAt)),'<span class=\"chip\">'+h(ago(r.submittedAt))+'</span>');}",
          "var html='<div class=\"wrap\">';",
    ...
    // ── 5. Annual Client Review ──────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "for(var j=0;j<ul.length;j++){var u=ul[j];var col=u.dd<0?'var(--fl-bad)':u.dd<=14?'var(--fl-warn)':'var(--ax)';q+=qRow(col,'Review due '+h(fmtDate(u.a.next_review_date)),'AUM '+h(mnyC(u.a.current_aum))+' \\u00b7 goal '+h(fmtInt(num(u.a.goal_progress)))+'/10','<span class=\"chip\">'+h(ago(u.a.next_review_date))+'</span>');}",
          "var gmax=Math.max(behind,onTrack,ahead);",
          "var gp=(behind+onTrack+ahead)?barRow('Exceeding',ahead,gmax,'var(--fl-good)')+barRow('On track',onTrack,gmax,'var(--ax)')+barRow('Behind',behind,gmax,'var(--fl-bad)'):'';",
          "var html='<div class=\"wrap\">';",
    ...
    // ── 6. Fee Agreement ─────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var under=sumBy(rows,'account_value');var fees=sumBy(rows,'calculated_annual_fee');",
          "var list='';var rl=rows.slice(0,6);",
    ...
    // ── 7. Document Vault ────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var exp=[];var m30=0;",
          "for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var dd=dayDiff(a.expiry_date);if(dd!=null&&dd<=60)exp.push({a:a,dd:dd});var sd=dayDiff(rows[i].submittedAt);if(sd!=null&&sd>-30)m30++;}",
          "exp.sort(function(x,y){return x.dd-y.dd;});",
          "var q='';var el=exp.slice(0,6);",
          "for(var j=0;j<el.length;j++){var e2=el[j];var col=e2.dd<0?'var(--fl-bad)':e2.dd<=30?'var(--fl-warn)':'var(--ax)';q+=qRow(col,h(String(e2.a.document_name||'Document')),h(labelFor(dtMap,e2.a.document_type))+' \\u00b7 '+(e2.dd<0?'expired ':'expires ')+h(fmtDate(e2.a.expiry_date)),'<span class=\"chip\">'+h(ago(e2.a.expiry_date))+'</span>');}",
          ...
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(I.box,ctx.title||'Document Vault',brief(['<b>'+fmtInt(rows.length)+'</b> documents on file',exp.length?'<b>'+fmtInt(exp.length)+'</b> expiring within 60 days':'nothing expiring soon','<b>'+fmtInt(m30)+'</b> added in 30 days']),'New document');",
          "html+='<div class=\"kpis\">'+kpi('On file',rows.length,'',{hero:true,spark:spark(weekly(rows))})+kpi('Expiring in 60d',exp.length,exp.length?pill('Renew soon','warn'):pill('All current','good'))+kpi('Added in 30d',m30,'new documents filed')+'</div>';",
          "html+=sec('Compliance files')+'<div class=\"grid g21\">'+panel('Expiring soon',qBody)+panel('Document types',breakdown(rows,'document_type',dtMap)||emptyBlock(I.chart,'No documents yet.','+ Add a document'))+'</div>';",
          "html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 9. Beneficiary Designation ───────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var d=pd(rows[i].submittedAt);if(d&&d.getTime()>=qs)qtr++;if(String(a.contingent_name_1||'').trim())withCont++;}",
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};var extra=0;if(String(an.primary_name_2||'').trim())extra++;if(String(an.primary_name_3||'').trim())extra++;var t=h(String(an.primary_name_1||'Designation'))+(extra?' +'+extra:'');list+=rowItem(t,h(String(an.account||''))+' \\u00b7 '+h(String(an.primary_relationship_1||''))+' \\u00b7 '+h(fmtDate(r.submittedAt)),'<span class=\"amt\">'+h(fmtInt(num(an.primary_percentage_1)))+'%</span>');}",
          ...
          "html+=sec('Estate directives')+'<div class=\"grid g21\">'+panel('Recent designations',list?'<div class=\"rows\">'+list+'</div>':emptyBlock(I.doc,'No designations on file yet.','+ Record the first designation'))+panel('Primary relationships',breakdown(rows,'primary_relationship_1',{})||emptyBlock(I.user,'No beneficiaries named yet.'))+'</div>';",
          "html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 11. 1035 Exchange ────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "var rows=await recs(500);",
          "var etMap=optionMap(ctx.fields,'exchange_type');",
          "var val=sumBy(rows,'estimated_value');var sur=sumBy(rows,'surrender_charges');",
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};list+=rowItem(h(String(an.new_carrier||'Exchange')),'from '+h(String(an.existing_carrier||'\\u2014'))+' \\u00b7 '+h(labelFor(etMap,an.exchange_type)),'<span class=\"amt\">'+h(mnyC(an.estimated_value))+'</span>');}",
    ...
    // ── 12. Rollover Form ────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "var rows=await recs(500);",
          "var rtMap=optionMap(ctx.fields,'rollover_type');var diMap=optionMap(ctx.fields,'direct_or_indirect');",
          ...
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};list+=rowItem(h(String(an.receiving_custodian||'Rollover')),'from '+h(String(an.current_custodian||'\\u2014'))+' \\u00b7 '+h(labelFor(diMap,an.direct_or_indirect)),'<span class=\"amt\">'+h(mnyC(an.estimated_value))+'</span>');}",
