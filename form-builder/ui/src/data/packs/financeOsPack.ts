...
export interface PackForm {
  ...
  customScreen?: { enabled?: boolean; html?: string; css?: string; js?: string; allowNewResponses?: boolean };
...
// ── Section-screen kit (shared by every per-form customScreen) ──────────────
...
export const SECTION_CSS = [
  ...
  '@media(min-width:961px){.kpis{grid-template-columns:1.9fr 1fr 1fr;}}',
].join('\n');
    ...
    // ── 1. New Client Onboarding ─────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var objMap=optionMap(ctx.fields,'investment_objectives');",
          "function isAcc(a){var v=String(a.accredited_investor||'').toLowerCase();if(v==='yes')return true;if(v==='no')return false;return num(a.annual_income)>200000||num(a.net_worth)>1000000;}",
          "var accredited=0,totalNw=0,recent30=0;",
          "for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};if(isAcc(a))accredited++;totalNw+=num(a.net_worth);var dd=dayDiff(rows[i].submittedAt);if(dd!=null&&dd>-30)recent30++;}",
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};var nm=(String(an.first_name||'')+' '+String(an.last_name||'')).trim()||'Client';var right=(isAcc(an)?pill('Accredited','good'):pill('Retail','neutral'))+'<span class=\"chip\">'+h(ago(r.submittedAt))+'</span>';list+=rowItem(h(nm),h(labelFor(objMap,an.investment_objectives))+' \\u00b7 '+h(fmtDate(r.submittedAt)),right);}",
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(I.user,ctx.title||'New Client Onboarding',brief(['<b>'+fmtInt(rows.length)+'</b> clients on the book','<b>'+fmtInt(accredited)+'</b> accredited','<b>'+fmtInt(recent30)+'</b> added in 30 days']),'New client');",
          "html+='<div class=\"kpis\">'+kpi('Clients',rows.length,'<b>'+fmtInt(recent30)+'</b> added in the last 30 days',{hero:true,spark:spark(weekly(rows))})+kpi('Accredited',accredited,rows.length?fmtInt(Math.round(accredited/rows.length*100))+'% of the book':'')+kpi('Total net worth',mnyC(totalNw),'reported across all clients')+'</div>';",
          "html+=sec('Book roster')+'<div class=\"grid g21\">'+panel('Investment objectives',breakdown(rows,'investment_objectives',objMap)||emptyBlock(I.chart,'No clients on the book yet.','+ Onboard the first client'))+panel('Recent clients',list?'<div class=\"rows\">'+list+'</div>':emptyBlock(I.user,'No clients yet.','+ Onboard a client'))+'</div>';",
    ...
    // ── 2. Risk Tolerance Questionnaire ──────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "var rows=await recs(500);",
          "var ptMap=optionMap(ctx.fields,'portfolio_type');var exMap=optionMap(ctx.fields,'investment_experience');",
          "function scoreOf(a){var s=num(a.risk_profile_score);if(s>0)return Math.max(1,Math.min(100,Math.round(s)));var lc=num(a.loss_capacity),th=num(a.time_horizon),ag=num(a.client_age);if(lc||th||ag)return Math.max(1,Math.min(100,Math.round(lc*5+th*2+(ag?(120-ag)/2:20))));var mr={sell:1,sell_some:4,hold:7,buy_more:10}[String(a.market_reaction||'')]||0;var ex={none:2,limited:4,moderate:7,extensive:10}[String(a.investment_experience||'')]||0;if(mr||ex)return Math.max(1,Math.min(100,Math.round(mr*6+ex*4)));return 0;}",
          "function flagOf(a){var v=String(a.reg_bi_check||'');if(v)return v==='Review needed';var s=scoreOf(a);var pt=String(a.portfolio_type||'');return (s>=70&&(pt==='conservative'||pt==='moderate'))||(s>0&&s<30&&(pt==='aggressive'||pt==='speculative'));}",
          "var flagged=0,scoreSum=0,scoreN=0;",
          "for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};if(flagOf(a))flagged++;var s=scoreOf(a);if(s>0){scoreSum+=s;scoreN++;}}",
          "var avg=scoreN?Math.round(scoreSum/scoreN):0;",
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};var flag=flagOf(an);var sc=scoreOf(an);var right=(flag?pill('Review needed','bad'):pill('Suitable','good'))+'<span class=\"chip\">score '+(sc?fmtInt(sc):'\\u2014')+'</span>';list+=rowItem(h(labelFor(ptMap,an.portfolio_type))+' portfolio',h(labelFor(exMap,an.investment_experience))+' \\u00b7 '+h(fmtDate(r.submittedAt)),right);}",
          "var glyph=icoSvg('<path d=\"M4.5 19a8.5 8.5 0 1 1 15 0\"/><path d=\"M12 13l3.5-3.5\"/><circle cx=\"12\" cy=\"13\" r=\"1.4\"/>');",
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(glyph,ctx.title||'Risk Tolerance Questionnaire',brief(['<b>'+fmtInt(rows.length)+'</b> assessments',flagged?'<b>'+fmtInt(flagged)+'</b> flagged for review':'none flagged',scoreN?'avg score <b>'+fmtInt(avg)+'</b>':'']),'New assessment');",
          "html+='<div class=\"kpis\">'+kpi('Assessments',rows.length,'',{hero:true,spark:spark(weekly(rows))})+kpi('Flagged',flagged,flagged?pill('Reg BI review','bad'):pill('All suitable','good'))+kpi('Avg score',scoreN?avg:'\\u2014','suitability score, 1\\u2013100')+'</div>';",
          "html+=sec('Suitability desk')+'<div class=\"grid g21\">'+panel('Recommended portfolio types',breakdown(rows,'portfolio_type',ptMap)||emptyBlock(I.chart,'No assessments completed yet.','+ Run the first assessment'))+panel('Recent assessments',list?'<div class=\"rows\">'+list+'</div>':emptyBlock(I.doc,'No assessments yet.','+ New assessment'))+'</div>';",
          "html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 3. ACAT / Transfer Form ──────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "var rows=await recs(500);",
          "var custMap=optionMap(ctx.fields,'custodian');var ttMap=optionMap(ctx.fields,'transfer_type');",
          "var inTransit=sumBy(rows,'estimated_value');var m30=0;",
          "for(var i=0;i<rows.length;i++){var dd=dayDiff(rows[i].submittedAt);if(dd!=null&&dd>-30)m30++;}",
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};list+=rowItem(h(labelFor(custMap,an.custodian)),h(labelFor(ttMap,an.transfer_type))+' transfer \\u00b7 '+h(fmtDate(r.submittedAt)),'<span class=\"amt\">'+h(mnyC(an.estimated_value))+'</span>');}",
          "var glyph=icoSvg('<path d=\"M5 12h14M13 6l6 6-6 6\"/>');",
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(glyph,ctx.title||'ACAT / Transfer Form',brief(['<b>'+fmtInt(rows.length)+'</b> transfers','<b>'+mnyC(inTransit)+'</b> in motion','<b>'+fmtInt(m30)+'</b> initiated in 30 days']),'New transfer');",
          "html+='<div class=\"kpis\">'+kpi('Transfers',rows.length,'',{hero:true,spark:spark(weekly(rows))})+kpi('In transit',mnyC(inTransit),'estimated value being moved')+kpi('Last 30 days',m30,'transfers initiated')+'</div>';",
          "html+=sec('Assets in motion')+'<div class=\"grid g21\">'+panel('Delivering custodians',breakdown(rows,'custodian',custMap)||emptyBlock(I.chart,'No transfers initiated yet.','+ Start the first transfer'))+panel('Recent transfers',list?'<div class=\"rows\">'+list+'</div>':emptyBlock(I.doc,'No transfers yet.','+ New transfer'))+'</div>';",
          "html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 4. Form CRS & Relationship Summary ───────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "var rows=await recs(500);",
          "var m30=0;var advSet={};",
          "for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var dd=dayDiff(rows[i].submittedAt);if(dd!=null&&dd>-30)m30++;var nm=String(a.advisor_name||'').trim();if(nm)advSet[nm]=1;}",
          "var advisors=Object.keys(advSet).length;",
          "var rl=rows.slice().sort(function(x,y){var xa=pd((x.answers||{}).acknowledgement_date)||pd(x.submittedAt);var ya=pd((y.answers||{}).acknowledgement_date)||pd(y.submittedAt);return (ya?ya.getTime():0)-(xa?xa.getTime():0);}).slice(0,6);",
          "var list='';",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};var ad=an.acknowledgement_date||r.submittedAt;list+=rowItem(h(String(an.advisor_name||'').trim()||'Acknowledgement'),'Acknowledged '+h(fmtDate(ad)),'<span class=\"chip\">'+h(ago(ad))+'</span>');}",
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(I.doc,ctx.title||'Form CRS',brief(['<b>'+fmtInt(rows.length)+'</b> disclosures delivered','<b>'+fmtInt(m30)+'</b> in the last 30 days','<b>'+fmtInt(advisors)+'</b> advisors delivering']),'New acknowledgement');",
          "html+='<div class=\"kpis\">'+kpi('Delivered',rows.length,'signed Form CRS acknowledgements',{hero:true,spark:spark(weekly(rows))})+kpi('Last 30 days',m30,'new acknowledgements')+kpi('Advisors',advisors,'delivering disclosures')+'</div>';",
          "html+=sec('Disclosure log')+'<div class=\"grid g21\">'+panel('Recent acknowledgements',list?'<div class=\"rows\">'+list+'</div>':emptyBlock(I.doc,'No acknowledgements on file yet.','+ Record the first acknowledgement'))+panel('By advisor',breakdown(rows,'advisor_name',{})||emptyBlock(I.user,'No advisor activity yet.'))+'</div>';",
          "html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 5. Annual Client Review ──────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "var rows=await recs(500);",
          "var aum=sumBy(rows,'current_aum');var due45=0;var up=[];var behind=0,onTrack=0,ahead=0;",
          "for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var dd=dayDiff(a.next_review_date);if(dd!=null){if(dd<=45)due45++;up.push({a:a,dd:dd});}var g=num(a.goal_progress);if(g>=8)ahead++;else if(g>=4)onTrack++;else if(g>=1)behind++;}",
          "up.sort(function(x,y){return x.dd-y.dd;});",
          "var q='';var ul=up.slice(0,6);",
          "for(var j=0;j<ul.length;j++){var u=ul[j];var col=u.dd<0?'var(--fl-bad)':u.dd<=14?'var(--fl-warn)':'var(--ax)';var unm=String(u.a.client_name||'').trim()||mnyC(u.a.current_aum)+' portfolio';q+=qRow(col,h(unm),(u.dd<0?'Overdue since ':'Review due ')+h(fmtDate(u.a.next_review_date))+' \\u00b7 goal '+h(fmtInt(num(u.a.goal_progress)))+'/10','<span class=\"amt\">'+h(mnyC(u.a.current_aum))+'</span><span class=\"chip\">'+h(ago(u.a.next_review_date))+'</span>');}",
          "var gmax=Math.max(behind,onTrack,ahead);",
          "var gp='';if(ahead)gp+=barRow('Exceeding',ahead,gmax,'var(--fl-good)');if(onTrack)gp+=barRow('On track',onTrack,gmax,'var(--ax)');if(behind)gp+=barRow('Behind',behind,gmax,'var(--fl-bad)');",
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(I.cal,ctx.title||'Annual Client Review',brief(['<b>'+fmtInt(rows.length)+'</b> reviews on file',due45?'<b>'+fmtInt(due45)+'</b> due within 45 days':'none due soon','<b>'+mnyC(aum)+'</b> AUM reviewed']),'New review');",
          "html+='<div class=\"kpis\">'+kpi('Reviews',rows.length,'',{hero:true,spark:spark(weekly(rows))})+kpi('Due within 45d',due45,due45?pill('Schedule now','warn'):pill('All clear','good'))+kpi('Total AUM',mnyC(aum),'across reviewed accounts')+'</div>';",
          "html+=sec('Review desk')+'<div class=\"grid g21\">'+panel('Upcoming reviews',q||emptyBlock(I.cal,'No reviews scheduled yet.','+ Log the first review'))+panel('Goal progress',gp||emptyBlock(I.chart,'No goal data yet.'))+'</div>';",
          "html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 6. Fee Agreement ─────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "var rows=await recs(500);",
          "var tierMap=optionMap(ctx.fields,'fee_tier');var freqMap=optionMap(ctx.fields,'billing_frequency');",
          "function feeOf(a){var f=num(a.calculated_annual_fee);if(f>0)return f;var v=num(a.account_value);return v<=500000?v*0.01:v<=1000000?v*0.0085:v*0.007;}",
          "var under=0,fees=0;",
          "for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};under+=num(a.account_value);fees+=feeOf(a);}",
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};var tt=an.fee_tier?h(labelFor(tierMap,an.fee_tier))+' tier':'Advisory agreement';list+=rowItem(tt,h(labelFor(freqMap,an.billing_frequency))+' billing \\u00b7 effective '+h(fmtDate(an.effective_date)),'<span class=\"amt\">'+h(mnyC(an.account_value))+'</span>');}",
          "var glyph=icoSvg('<path d=\"M12 2.5v19\"/><path d=\"M16.5 6H9.75a3.25 3.25 0 0 0 0 6.5h4.5a3.25 3.25 0 0 1 0 6.5H7\"/>');",
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(glyph,ctx.title||'Fee Agreement',brief(['<b>'+fmtInt(rows.length)+'</b> agreements','<b>'+mnyC(under)+'</b> under agreement','<b>'+mnyC(fees)+'</b> in annual fees']),'New agreement');",
          "html+='<div class=\"kpis\">'+kpi('Agreements',rows.length,'',{hero:true,spark:spark(weekly(rows))})+kpi('Under agreement',mnyC(under),'total account value')+kpi('Est. annual fees',mnyC(fees),'calculated advisory fees')+'</div>';",
          "html+=sec('Fee schedule')+'<div class=\"grid g21\">'+panel('Fee tiers',breakdown(rows,'fee_tier',tierMap)||emptyBlock(I.chart,'No fee agreements yet.','+ Set up the first agreement'))+panel('Recent agreements',list?'<div class=\"rows\">'+list+'</div>':emptyBlock(I.doc,'No agreements yet.','+ New agreement'))+'</div>';",
          "html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 7. Document Vault ────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "var rows=await recs(500);",
          "var dtMap=optionMap(ctx.fields,'document_type');",
          "var exp=[];var m30=0,expired=0,soon=0;",
          "for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var dd=dayDiff(a.expiry_date);if(dd!=null&&dd<=60){exp.push({a:a,dd:dd});if(dd<0)expired++;else soon++;}var sd=dayDiff(rows[i].submittedAt);if(sd!=null&&sd>-30)m30++;}",
          "exp.sort(function(x,y){return x.dd-y.dd;});",
          "var q='';var el=exp.slice(0,6);",
          "for(var j=0;j<el.length;j++){var e2=el[j];var col=e2.dd<0?'var(--fl-bad)':e2.dd<=30?'var(--fl-warn)':'var(--ax)';q+=qRow(col,h(String(e2.a.document_name||'Document')),h(labelFor(dtMap,e2.a.document_type))+' \\u00b7 '+(e2.dd<0?'expired ':'expires ')+h(fmtDate(e2.a.expiry_date)),(e2.dd<0?pill('Expired','bad'):'')+'<span class=\"chip\">'+h(ago(e2.a.expiry_date))+'</span>');}",
          "var qBody=q||(rows.length?emptyBlock(I.check,'Nothing expires in the next 60 days.'):emptyBlock(I.doc,'No documents on file yet.','+ File the first document'));",
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(I.box,ctx.title||'Document Vault',brief(['<b>'+fmtInt(rows.length)+'</b> documents on file',expired?'<b>'+fmtInt(expired)+'</b> expired':'',soon?'<b>'+fmtInt(soon)+'</b> expiring within 60 days':(expired?'':'nothing expiring soon'),'<b>'+fmtInt(m30)+'</b> added in 30 days']),'New document');",
          "html+='<div class=\"kpis\">'+kpi('On file',rows.length,'',{hero:true,spark:spark(weekly(rows))})+kpi('Expiring in 60d',soon,expired?pill(fmtInt(expired)+' expired','bad'):(soon?pill('Renew soon','warn'):pill('All current','good')))+kpi('Added in 30d',m30,'new documents filed')+'</div>';",
          "html+=sec('Compliance files')+'<div class=\"grid g21\">'+panel('Expired & expiring soon',qBody)+panel('Document types',breakdown(rows,'document_type',dtMap)||emptyBlock(I.chart,'No documents yet.','+ Add a document'))+'</div>';",
          "html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 8. W-9 Form ─────────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "var rows=await recs(500);",
          "var tcMap=optionMap(ctx.fields,'tax_classification');",
          "var now=new Date();var qs=new Date(now.getFullYear(),Math.floor(now.getMonth()/3)*3,1).getTime();",
          "var qtr=0,biz=0;",
          "for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var d=pd(rows[i].submittedAt);if(d&&d.getTime()>=qs)qtr++;if(a.tax_classification&&a.tax_classification!=='individual')biz++;}",
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};list+=rowItem(h(String(an.legal_name||'W-9')),h(labelFor(tcMap,an.tax_classification))+' \\u00b7 '+h(fmtDate(r.submittedAt)),'<span class=\"chip\">'+h(ago(r.submittedAt))+'</span>');}",
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(I.doc,ctx.title||'W-9 Form',brief(['<b>'+fmtInt(rows.length)+'</b> W-9s on file','<b>'+fmtInt(qtr)+'</b> this quarter','<b>'+fmtInt(biz)+'</b> business entities']),'New W-9');",
          "html+='<div class=\"kpis\">'+kpi('On file',rows.length,'taxpayer certifications',{hero:true,spark:spark(weekly(rows))})+kpi('This quarter',qtr,'collected since quarter start')+kpi('Business entities',biz,'LLCs, corps, partnerships, trusts')+'</div>';",
          "html+=sec('Tax certifications')+'<div class=\"grid g21\">'+panel('Tax classifications',breakdown(rows,'tax_classification',tcMap)||emptyBlock(I.chart,'No W-9s collected yet.','+ Collect the first W-9'))+panel('Recent W-9s',list?'<div class=\"rows\">'+list+'</div>':emptyBlock(I.doc,'No W-9s yet.','+ New W-9'))+'</div>';",
          "html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 9. Beneficiary Designation ───────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "var rows=await recs(500);",
          "var txt=function(v){var s=(v==null?'':String(v)).trim();if(!s)return '';return /^(sample entry|ref[\\s-]?\\d+|[0-9a-fA-F]{10,})$/i.test(s)?'':s;};var relMap=optionMap(ctx.fields,'primary_relationship_1');",
          "var now=new Date();var qs=new Date(now.getFullYear(),Math.floor(now.getMonth()/3)*3,1).getTime();",
          "var qtr=0,withCont=0;",
          "for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var d=pd(rows[i].submittedAt);if(d&&d.getTime()>=qs)qtr++;if(txt(a.contingent_name_1))withCont++;}",
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};var nm=txt(an.primary_name_1);var extra=0;if(txt(an.primary_name_2))extra++;if(txt(an.primary_name_3))extra++;var t=(nm||'Beneficiary designation')+(extra?' +'+extra:'');var rel=txt(an.primary_relationship_1);var acct=txt(an.account);var sp=[];if(rel)sp.push(labelFor(relMap,an.primary_relationship_1));if(acct)sp.push(acct);sp.push(fmtDate(r.submittedAt));list+=rowItem(h(t),h(sp.join(' \\u00b7 ')),'<span class=\"amt\">'+h(fmtInt(num(an.primary_percentage_1)))+'%</span>');}",
          "var glyph=icoSvg('<path d=\"M12 20.5C7 16.5 3.5 13.3 3.5 9.6 3.5 7 5.5 5 8 5c1.6 0 3.1.8 4 2.1C12.9 5.8 14.4 5 16 5c2.5 0 4.5 2 4.5 4.6 0 3.7-3.5 6.9-8.5 10.9z\"/>');",
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(glyph,ctx.title||'Beneficiary Designation',brief(['<b>'+fmtInt(rows.length)+'</b> designations on file','<b>'+fmtInt(qtr)+'</b> this quarter','<b>'+fmtInt(withCont)+'</b> with contingents']),'New designation');",
          "html+='<div class=\"kpis\">'+kpi('Designations',rows.length,'beneficiary elections on file',{hero:true,spark:spark(weekly(rows))})+kpi('This quarter',qtr,'signed since quarter start')+kpi('With contingents',withCont,'name a backup beneficiary')+'</div>';",
          "html+=sec('Estate directives')+'<div class=\"grid g21\">'+panel('Recent designations',list?'<div class=\"rows\">'+list+'</div>':emptyBlock(I.doc,'No designations on file yet.','+ Record the first designation'))+panel('Primary relationships',breakdown(rows,'primary_relationship_1',relMap)||emptyBlock(I.user,'No beneficiaries named yet.'))+'</div>';",
          "html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 10. Power of Attorney ────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "var rows=await recs(500);",
          "var pgMap=optionMap(ctx.fields,'powers_granted');",
          "var m30=0,notarized=0;",
          "for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var dd=dayDiff(rows[i].submittedAt);if(dd!=null&&dd>-30)m30++;var n=a.notary;if(n!=null&&n!==''&&!(Array.isArray(n)&&n.length===0))notarized++;}",
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};list+=rowItem(h(String(an.principal_name||'Power of attorney')),'Agent '+h(String(an.agent_name||'\\u2014'))+' \\u00b7 effective '+h(fmtDate(an.effective_date)),'<span class=\"chip\">'+h(ago(r.submittedAt))+'</span>');}",
          "var glyph=icoSvg('<path d=\"M12 3l7.5 2.8v5.4c0 4.3-3 7.7-7.5 9-4.5-1.3-7.5-4.7-7.5-9V5.8z\"/>');",
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(glyph,ctx.title||'Power of Attorney',brief(['<b>'+fmtInt(rows.length)+'</b> POAs on file','<b>'+fmtInt(m30)+'</b> added in 30 days','<b>'+fmtInt(notarized)+'</b> notarized']),'New POA');",
          "html+='<div class=\"kpis\">'+kpi('POAs on file',rows.length,'executed authorities',{hero:true,spark:spark(weekly(rows))})+kpi('Added in 30d',m30,'recently executed')+kpi('Notarized',notarized,'with a notary document attached')+'</div>';",
          "html+=sec('Authority on file')+'<div class=\"grid g21\">'+panel('Powers granted',breakdown(rows,'powers_granted',pgMap)||emptyBlock(I.chart,'No powers of attorney on file yet.','+ Record the first POA'))+panel('Recent POAs',list?'<div class=\"rows\">'+list+'</div>':emptyBlock(I.doc,'No POAs yet.','+ New POA'))+'</div>';",
          "html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 11. 1035 Exchange ────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "var rows=await recs(500);",
          "var txt=function(v){var s=(v==null?'':String(v)).trim();if(!s)return '';return /^(sample entry|ref[\\s-]?\\d+|[0-9a-fA-F]{10,})$/i.test(s)?'':s;};var etMap=optionMap(ctx.fields,'exchange_type');var ecMap=optionMap(ctx.fields,'existing_carrier');var ncMap=optionMap(ctx.fields,'new_carrier');",
          "var val=sumBy(rows,'estimated_value');var sur=sumBy(rows,'surrender_charges');",
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};var fc=txt(labelFor(ecMap,an.existing_carrier)),tc=txt(labelFor(ncMap,an.new_carrier));var sp=[];if(fc)sp.push('from '+fc);if(tc)sp.push('to '+tc);if(!sp.length)sp.push(labelFor(etMap,an.exchange_type)+' policy');sp.push(fmtDate(r.submittedAt));list+=rowItem(h(labelFor(etMap,an.exchange_type))+' exchange',h(sp.join(' \\u00b7 ')),'<span class=\"amt\">'+h(mnyC(an.estimated_value))+'</span>');}",
          "var glyph=icoSvg('<path d=\"M6 3h12v18l-2-1.5-2 1.5-2-1.5L10 21l-2-1.5L6 21z\"/><path d=\"M9.5 8h5M9.5 12h5\"/>');",
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(glyph,ctx.title||'1035 Exchange',brief(['<b>'+fmtInt(rows.length)+'</b> exchanges','<b>'+mnyC(val)+'</b> in contract value',sur?'<b>'+mnyC(sur)+'</b> surrender charges':'no surrender charges']),'New exchange');",
          "html+='<div class=\"kpis\">'+kpi('Exchanges',rows.length,'',{hero:true,spark:spark(weekly(rows))})+kpi('Contract value',mnyC(val),'policies being exchanged')+kpi('Surrender charges',mnyC(sur),sur?pill('Verify before transfer','warn'):'none reported')+'</div>';",
          "html+=sec('Exchanges in motion')+'<div class=\"grid g21\">'+panel('Exchange types',breakdown(rows,'exchange_type',etMap)||emptyBlock(I.chart,'No 1035 exchanges initiated yet.','+ Start the first exchange'))+panel('Recent exchanges',list?'<div class=\"rows\">'+list+'</div>':emptyBlock(I.doc,'No exchanges yet.','+ New exchange'))+'</div>';",
          "html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
    ...
    // ── 12. Rollover Form ────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var ctx;try{ctx=await FL.context();}catch(e){return fatal(root,'Could not load this section.');}",
          "var rows=await recs(500);",
          "var txt=function(v){var s=(v==null?'':String(v)).trim();if(!s)return '';return /^(sample entry|ref[\\s-]?\\d+|[0-9a-fA-F]{10,})$/i.test(s)?'':s;};var rtMap=optionMap(ctx.fields,'rollover_type');",
          "var moving=sumBy(rows,'estimated_value');var direct=0,indirect=0;",
          "for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};if(a.direct_or_indirect==='direct')direct++;else if(a.direct_or_indirect==='indirect')indirect++;}",
          "var list='';var rl=rows.slice(0,6);",
          "for(var j=0;j<rl.length;j++){var r=rl[j],an=r.answers||{};var rc=txt(an.receiving_custodian);var sub=(rc?'to '+rc+' \\u00b7 ':'')+(an.direct_or_indirect==='indirect'?'Indirect (60-day)':'Direct')+' \\u00b7 '+fmtDate(r.submittedAt);list+=rowItem(h(labelFor(rtMap,an.rollover_type)),h(sub),'<span class=\"amt\">'+h(mnyC(an.estimated_value))+'</span>');}",
          "var glyph=icoSvg('<rect x=\"3.5\" y=\"7\" width=\"17\" height=\"13\" rx=\"2.5\"/><path d=\"M3.5 9.5V6.8A1.8 1.8 0 0 1 5.3 5H17\"/><path d=\"M15.5 13.5h2\"/>');",
          "var html='<div class=\"wrap\">';",
          "html+=headerBlock(glyph,ctx.title||'Rollover Form',brief(['<b>'+fmtInt(rows.length)+'</b> rollovers','<b>'+mnyC(moving)+'</b> in motion',indirect?'<b>'+fmtInt(indirect)+'</b> indirect (60-day clock)':'all direct']),'New rollover');",
          "html+='<div class=\"kpis\">'+kpi('Rollovers',rows.length,'',{hero:true,spark:spark(weekly(rows))})+kpi('Moving',mnyC(moving),'estimated retirement assets')+kpi('Direct',direct,indirect?pill(fmtInt(indirect)+' indirect (60-day)','warn'):'no indirect rollovers')+'</div>';",
          "html+=sec('Retirement transfers')+'<div class=\"grid g21\">'+panel('Rollover types',breakdown(rows,'rollover_type',rtMap)||emptyBlock(I.chart,'No rollovers initiated yet.','+ Start the first rollover'))+panel('Recent rollovers',list?'<div class=\"rows\">'+list+'</div>':emptyBlock(I.doc,'No rollovers yet.','+ New rollover'))+'</div>';",
          "html+='</div>';root.innerHTML=html;wire(root);",
          "}",
          "main();",
        ].join('\n'),
