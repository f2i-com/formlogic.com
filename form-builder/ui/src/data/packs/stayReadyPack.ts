    ...
    // ── 3. Booking ────────────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "      tHtml+='<div class=\"sch\"><span class=\"sch-t num\">'+(e.t==='in'?'In':'Out')+'</span><div class=\"row-main\"><div class=\"row-title\">'+h(e.a.guest_name||'Guest')+'</div><div class=\"row-sub\">'+h(labelFor(pfMap,e.a.platform))+(gc>0?' \\u00b7 '+fmtInt(gc)+' guest'+(gc===1?'':'s'):'')+'</div></div><div class=\"row-r\">'+stPill(stMap,e.a.status)+'</div></div>';}",
          "    if(open)tHtml+='</div>';}",
          "  else if(rows.length){",
    ...
    // ── 6. Supply Item ────────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "  for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};",
          "    if(a.status==='low')low++;else if(a.status==='out')out++;else if(a.status==='ok')ok++;",
          "    if(a.status==='low'||a.status==='out'){var pid=Array.isArray(a.property)?a.property[0]:a.property;if(pid)propSet[pid]=1;}",
          "    var cs=num(a.current_stock),rp=num(a.reorder_point);",
          "    items.push({a:a,cs:cs,rp:rp,ratio:rp>0?cs/rp:(cs>0?2:0)});}",
          ...
          "    pHtml+=barRow(it.a.supply_item||'Supply item',it.cs,Math.max(it.cs,it.rp>0?it.rp:(it.cs||1)),BC[it.a.status]||'var(--fl-accent)');}",
  ...
  apps: [
    {
      ...
      customScreen: {
        ...
        js: [
          ...
          "  html+=sec('Turnover runway')+'<div class=\"grid g21\">'+panel('Today & upcoming',runHtml,fT?'form/'+fT.formId+'/responses':'')+panel('Same-day flips',flipHtml,fB?'form/'+fB.formId+'/responses':'')+'</div>';",
          "  var hHtml='';",
