    ...
    // ── 6. Escalation Form ────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "  else{rec='<div class=\"rows\">';for(var ri=0;ri<Math.min(6,rows.length);ri++){var a=rows[ri].answers||{};var uv=a.urgency;var kind=(uv==='critical'||uv==='high')?'bad':(uv==='medium')?'warn':'neutral';rec+=rowItem(h(a.escalated_to?'To '+a.escalated_to:'Escalation'),h(labelFor(reasonMap,a.escalation_reason))+' \\u00b7 '+fmtDate(a.escalation_date||rows[ri].submittedAt),pill(urgShort(urgMap,uv),kind));}rec+='</div>';}",
    ...
    // ── 7. Knowledge Base Entry ───────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "  else{rec='<div class=\"rows\">';for(var ri=0;ri<Math.min(6,rows.length);ri++){var a=rows[ri].answers||{};var sv=a.status;var kind=sv==='published'?'good':(sv==='needs_review')?'warn':'neutral';rec+=rowItem(h(a.title||'Article'),h(labelFor(catMap,a.category))+' \\u00b7 '+fmtDate(a.last_reviewed||rows[ri].submittedAt),pill(labelFor(stMap,sv),kind));}rec+='</div>';}",
