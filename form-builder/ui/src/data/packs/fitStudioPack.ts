    ...
    // ── 3. Session ────────────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "  else if(rows.length){",
          "    var latest=rows.slice().sort(function(x,y){var xd=pd((x.answers||{}).date),yd=pd((y.answers||{}).date);return (yd?yd.getTime():0)-(xd?xd.getTime():0);});",
          "    for(var p=0;p<Math.min(6,latest.length);p++){var r2=latest[p].answers||{};",
  ...
  apps: [
    {
      ...
      customScreen: {
        ...
        js: [
          ...
          "    if(pa.status==='paid'&&pdt){if((Date.now()-pdt.getTime())<=2592000000){revMonth+=num(pa.amount);paidMonthN++;}var pwk=Math.floor((Date.now()-pdt.getTime())/604800000);if(pwk>=0&&pwk<8)revSpark[7-pwk]+=num(pa.amount);}",
          ...
          "  var html='<div class=\"wrap\">';",
          "  html+=headerBlock(DB,ctx.appName||'FitStudio',brief(['<b>'+fmtInt(todayN)+'</b> session'+(todayN===1?'':'s')+' today',nextClause,'<b>'+mnyC(revMonth)+'</b> collected in 30 days',chase>0?'<b>'+fmtInt(chase)+'</b> payment'+(chase===1?'':'s')+' to chase':'']),[{nav:fS?fS.formId:'',label:'Book session',icon:I.cal},{nav:fC?fC.formId:'',label:'Add client',icon:I.user,ghost:true}]);",
          "  html+='<div class=\"kpis\">'",
          "    +kpi('Collected 30d',mnyC(revMonth),paidMonthN>0?'<b>'+fmtInt(paidMonthN)+'</b> payment'+(paidMonthN===1?'':'s')+' cleared':(payments.length?'no cleared payments in 30 days':'record your first payment'),{hero:true,spark:revNz>=2?spark(revSpark):''})",
          ...
          "  else if(bookMode==='up'){var open=false,lastD=null;",
          ...
          "    if(open)bookHtml+='</div>';}",
          "  else{for(var b3=0;b3<list.length;b3++){var e2=list[b3];",
          "      var ty2=e2.a.session_type?labelFor(sesTyMap,e2.a.session_type):'Session';var tr2=refName(tn,e2.a.trainer);",
          "      bookHtml+=rowItem(h(refName(cn,e2.a.client)||'Session'),h(ty2+(tr2?' \\u00b7 with '+tr2:'')+' \\u00b7 '+fmtDate(e2.a.date)),sesPill(sesStMap,e2.a.status));}",
          "    bookHtml='<div class=\"rows\">'+bookHtml+'</div>';}",
          "  var chaseSorted=[];",
