    ...
    // ── 6. Payment ────────────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "  var coll30=0,paid30=0,pendN=0,pendSum=0,failN=0;",
          "  for(var i=0;i<rows.length;i++){var a=rows[i].answers||{};var st=a.status;var dd=dayDiff(a.date);",
          "    if(st==='paid'&&dd!=null&&dd>=-30&&dd<=0){coll30+=num(a.amount);paid30++;}",
          ...
          "  html+='<div class=\"kpis\">'",
          "    +kpi('Collected 30d',mnyC(coll30),paid30>0?'<b>'+fmtInt(paid30)+'</b> payment'+(paid30===1?'':'s')+' cleared':(rows.length?'no cleared payments in 30d':'record your first payment'),{hero:true,spark:spark(weekly(rows))})",
  ...
  apps: [
    {
      ...
      customScreen: {
        ...
        js: [
          ...
          "  for(var i=0;i<sessions.length;i++){var a=sessions[i].answers||{};var st=a.status;var dd=dayDiff(a.date);",
          "    if(dd===0&&st!=='cancelled'){todayN++;var tm=String(a.time||'');if(tm&&(nextT===null||tm<nextT))nextT=tm;}",
          "    if(dd!=null&&dd>=0&&dd<=6&&st!=='cancelled')weekN++;",
          ...
          "  var revMonth=0,paidMonthN=0,pendN=0,failN=0,pendSum=0;",
          "  for(var p=0;p<payments.length;p++){var pa=payments[p].answers||{};var pdt=pd(pa.date);",
          "    if(pa.status==='paid'&&pdt&&pdt.getTime()>=mS){revMonth+=num(pa.amount);paidMonthN++;}",
          ...
          "  html+='<div class=\"kpis\">'",
          "    +kpi('Collected this month',mnyC(revMonth),paidMonthN>0?'<b>'+fmtInt(paidMonthN)+'</b> payment'+(paidMonthN===1?'':'s')+' cleared':(payments.length?'no cleared payments yet':'record your first payment'),{hero:true,spark:spark(weekly(payments))})",
