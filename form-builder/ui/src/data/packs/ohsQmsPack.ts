    ...
    // ── 1. Incident Report ──────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var recent='';",
          "for(var r=0;r<rows.length&&r<6;r++){var a=rows[r].answers||{};recent+=rowItem(h(a.incident_title||'Incident'),h(labelFor(tyMap,a.incident_type))+' \\u00b7 '+h(fmtDate(a.incident_date||rows[r].submittedAt)),pill(labelFor(sevMap,a.severity),sevKind(a.severity)));}",
    ...
    // ── 2. Injury Record ────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var recent='';",
          "for(var r=0;r<rows.length&&r<6;r++){var a=rows[r].answers||{};var right='<span class=\"chip\">'+h(fmtDate(a.injury_date||rows[r].submittedAt))+'</span>'+(a.lost_time==='yes'?pill('Lost time','bad'):'');recent+=rowItem(h(a.employee_name||'Employee'),h(labelFor(tyMap,a.injury_type))+' \\u00b7 '+h(a.body_part||'\\u2014'),right);}",
    ...
    // ── 4. Action Item ──────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var queue='';",
          "for(var b=0;b<q.length&&b<7;b++){var ba=q[b].answers||{};var dd=dayDiff(ba.due_date);var col=dd!=null&&dd<0?'var(--fl-bad)':dd!=null&&dd<=7?'var(--fl-warn)':'var(--fl-accent)';var right=(ba.due_date?'<span class=\"chip\">due '+h(ago(ba.due_date))+'</span>':'')+pill(labelFor(prMap,ba.priority),prKind(ba.priority));queue+=qRow(col,h(ba.action_title||'Action'),h(labelFor(srcMap,ba.source_type))+' \\u00b7 '+h(ba.assigned_to||'Unassigned'),right);}",
          "if(!queue)queue=emptyBlock(I.check,'Actions queue up here, due-date first.','+ New action');",
          "var recent='';",
          "for(var r=0;r<rows.length&&r<5;r++){var a=rows[r].answers||{};recent+=rowItem(h(a.action_title||'Action'),h(a.assigned_to||'Unassigned')+' \\u00b7 '+h(fmtDate(rows[r].submittedAt)),pill(labelFor(prMap,a.priority),prKind(a.priority)));}",
    ...
    // ── 5. Audit Report ─────────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var recent='';",
          "for(var r=0;r<rows.length&&r<6;r++){var a=rows[r].answers||{};var right='<span class=\"chip\">'+h(fmtDate(a.audit_date||rows[r].submittedAt))+'</span>'+(a.finding_severity?pill(labelFor(fsMap,a.finding_severity),fsKind(a.finding_severity)):'');recent+=rowItem(h(a.audit_title||'Audit'),h(labelFor(tyMap,a.audit_type))+' \\u00b7 '+h(a.auditor_name||'\\u2014'),right);}",
    ...
    // ── 6. Corrective Action Request (CAR) ──────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "q.sort(function(x,y){return String((x.answers||{}).due_date||'9999').localeCompare(String((y.answers||{}).due_date||'9999'));});",
          "var queue='';",
          "for(var b=0;b<q.length&&b<7;b++){var ba=q[b].answers||{};var dd=dayDiff(ba.due_date);var col=dd!=null&&dd<0?'var(--fl-bad)':dd!=null&&dd<=7?'var(--fl-warn)':'var(--fl-accent)';var right=(ba.due_date?'<span class=\"chip\">due '+h(ago(ba.due_date))+'</span>':'')+pill(labelFor(prMap,ba.priority),prKind(ba.priority));queue+=qRow(col,h(ba.car_title||'CAR'),h(labelFor(srcMap,ba.source_type))+' \\u00b7 '+h(ba.assigned_to||'Unassigned'),right);}",
    ...
    // ── 7. Non-Conformance Report (NCR) ─────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var recent='';",
          "for(var r=0;r<rows.length&&r<6;r++){var a=rows[r].answers||{};recent+=rowItem(h(a.ncr_title||'NCR'),h(labelFor(tyMap,a.ncr_type))+' \\u00b7 '+h(fmtDate(a.raised_date||rows[r].submittedAt)),pill(labelFor(svMap,a.severity),svKind(a.severity)));}",
    ...
    // ── 8. Complaint Report ─────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var recent='';",
          "for(var r=0;r<rows.length&&r<6;r++){var a=rows[r].answers||{};recent+=rowItem(h(a.complaint_title||'Complaint'),h(labelFor(tyMap,a.complaint_type))+' \\u00b7 '+h(fmtDate(a.received_date||rows[r].submittedAt)),pill(labelFor(prMap,a.priority),prKind(a.priority)));}",
    ...
    // ── 9. Training Record ──────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var recent='';",
          "for(var r=0;r<rows.length&&r<6;r++){var a=rows[r].answers||{};var right='<span class=\"chip\">'+h(fmtDate(a.completion_date||rows[r].submittedAt))+'</span>'+(a.assessment_result&&a.assessment_result!=='na'?pill(labelFor(arMap,a.assessment_result),arKind(a.assessment_result)):'');recent+=rowItem(h(a.employee_name||'Employee'),h(a.course_name||'\\u2014'),right);}",
    ...
    // ── 10. Contractor Approval ─────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var recent='';",
          "for(var r=0;r<rows.length&&r<6;r++){var a=rows[r].answers||{};var right=(a.approval_expiry?'<span class=\"chip\">expires '+h(fmtDate(a.approval_expiry))+'</span>':'')+(a.induction_completed?pill(a.induction_completed==='yes'?'Inducted':labelFor(indMap,a.induction_completed),indKind(a.induction_completed)):'');recent+=rowItem(h(a.business_name||'Contractor'),h(a.contact_name||'\\u2014'),right);}",
    ...
    // ── 11. Meeting / Toolbox Talk ───────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var recent='';",
          "for(var r=0;r<rows.length&&r<6;r++){var a=rows[r].answers||{};var right='<span class=\"chip\">'+h(fmtDate(a.meeting_date||rows[r].submittedAt))+(a.meeting_time?' '+h(fmtTime(a.meeting_time)):'')+'</span>';recent+=rowItem(h(a.meeting_title||'Meeting'),h(labelFor(tyMap,a.meeting_type))+' \\u00b7 '+h(a.facilitator||'\\u2014'),right);}",
    ...
    // ── 12. Plant & Equipment Register ──────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var recent='';",
          "for(var r=0;r<rows.length&&r<6;r++){var a=rows[r].answers||{};var dd=dayDiff(a.next_service_date);var right=a.next_service_date?'<span class=\"chip\">service '+h(ago(a.next_service_date))+'</span>':'';if(dd!=null&&dd<0)right+=pill('Overdue','bad');else if(dd!=null&&dd<=30)right+=pill('Due soon','warn');recent+=rowItem(h(a.asset_name||'Asset'),h(labelFor(catMap,a.category))+' \\u00b7 '+h(a.equipment_location||'\\u2014'),right);}",
    ...
    // ── 1. Safety Management ────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: [
          ...
          "var ri='';for(i=0;i<Math.min(5,inc.length);i++){r=inc[i];a=r.answers||{};ri+=rowItem(h(a.incident_title||'Untitled incident'),h(a.location||'—')+' · '+h(fmtDate(a.incident_date)),pill(labelFor(sevL,a.severity),SEVP[a.severity]||'neutral'));}",
    ...
    // ── 2. Quality & Compliance ─────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: [
          ...
          "var cb='';for(i=0;i<Math.min(5,cmp.length);i++){r=cmp[i];a=r.answers||{};cb+=rowItem(h(a.complaint_title||'Untitled complaint'),h(labelFor(cmsL,a.complaint_source))+' · '+h(fmtDate(a.received_date)),pill(labelFor(priL,a.priority),priPill(a.priority)));}",
