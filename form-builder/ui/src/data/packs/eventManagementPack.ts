    ...
    // ── 1. Event Registration ─────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var tc=countBy(rows,'ticket_type');var topT=null,topN=0;for(var k in tc){if(tc[k]>topN){topN=tc[k];topT=k;}}",
          "var heroSub=topT?'<b>'+h(labelFor(tktMap,topT))+'</b> leads with '+Math.round(topN/rows.length*100)+'% of tickets':'Waiting on the first registration';",
    ...
    // ── 5. Post-Event Feedback ────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var recent='';for(i=0;i<Math.min(6,rows.length);i++){var a2=rows[i].answers||{};var rec2=a2.would_recommend;recent+=rowItem('<span style=\"color:var(--ax)\">'+stars(Math.round(num(a2.overall_rating)))+'</span>',h(labelFor(againMap,a2.would_attend_again))+' \\u00b7 '+fmtDate(rows[i].submittedAt),rec2?pill(labelFor(recMap,rec2),rec2==='yes'?'good':rec2==='no'?'bad':'warn'):'');}",
    ...
    // ── 7. Event Budget Item ──────────────────────────────────────────────
    {
      ...
      customScreen: {
        ...
        js: SECTION_KIT_JS + '\n' + [
          ...
          "var keys=Object.keys(cats);keys.sort(function(x,y){return cats[y].e-cats[x].e;});",
          "var body='';for(i=0;i<keys.length;i++){var k2=keys[i],cv=cats[k2];var pct=cv.e>0?Math.min(100,Math.round(cv.a/cv.e*100)):(cv.a>0?100:0);var nm=labelFor(catMap,k2)+(cv.a>cv.e?' \\u00b7 over':'');body+='<div class=\"bar-row\"><span class=\"bar-name\" title=\"'+h(nm)+'\">'+h(nm)+'</span><div class=\"bar-track\"><div class=\"bar-fill\" data-pct=\"'+pct+'\" style=\"background:'+(cv.a>cv.e?'var(--fl-bad)':'var(--fl-accent)')+'\"></div></div><span class=\"bar-val\">'+mnyC(cv.a)+'</span></div>';}",
          "if(!body)body=emptyBlock(I.chart,'Estimated vs actual spend by category will chart here.','+ Add a budget item');",
          "var recent='';for(i=0;i<Math.min(6,rows.length);i++){var ra=rows[i].answers||{};var ov=num(ra.actual_cost)>num(ra.estimated_cost);recent+=rowItem(h(ra.item_name||'Budget item'),h(labelFor(catMap,ra.category))+' \\u00b7 '+fmtDate(rows[i].submittedAt),'<span class=\"amt\">'+mnyC(ra.actual_cost!=null&&ra.actual_cost!==''?ra.actual_cost:ra.estimated_cost)+'</span>'+(ov?pill('Over','bad'):''));}",
