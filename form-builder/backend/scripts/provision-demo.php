...
function genValue(array $field, int $i, array $seeded, string $formName = '', int $count = 12)
{
    ...
    switch ($type) {
        case 'short_text':
            ...
            if ($has($label, ['incident title']) || ($has($formName, ['incident']) && $has($label, ['title']))) { return pickSeq($P['incidentTitles'], $formName, 'title', $i); }
            if ($has($label, ['hazard title']) || ($has($formName, ['hazard']) && $has($label, ['title']))) { return pickSeq($P['hazardTitles'], $formName, 'title', $i); }
...
function seedPools(): array
{
    ...
    $P = [
        ...
        'incidentTitles' => ['Slip on wet floor', 'Forklift near-miss in loading bay', 'Chemical spill in Bay 3', 'Fall from ladder', 'Trip on trailing cable', 'Manual handling back strain', 'Struck by falling stock', 'Laceration from box cutter', 'Burn from hot surface', 'Eye irritation from dust', 'Electric shock at switchboard', 'Reversing vehicle near-miss', 'Slip on stairwell', 'Pinch injury on conveyor'],
        'hazardTitles' => ['Blocked fire exit', 'Frayed extension lead', 'Wet floor near entrance', 'Unguarded machine belt', 'Cluttered walkway', 'Missing machine guard', 'Overloaded power board', 'Damaged ladder', 'Poor lighting in stairwell', 'Exposed wiring', 'Leaking chemical drum', 'Uneven floor surface', 'Faulty smoke alarm', 'Trailing cables across aisle'],
