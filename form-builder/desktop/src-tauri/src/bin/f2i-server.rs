...
async fn main() {
    ...
    // FormLogic Desktop plugin host (headless): same <dataDir>/plugins layout
    // as the GUI. Dev mode comes from FORMLOGIC_DEV_MODE=1 only — a headless
    ...
    // FormLogic Cloud link (headless): base URL + API key from the environment.
    // With both set, FormLogic Desktop becomes the HEADLESS runtime for flows +
    // the Aokie receptionist — polling/claiming queued runs, reserving event
    // bindings, and applying app-logic record writes without any browser.
    let fl_config = FormLogicConfig {
        base_url: std::env::var("FORMLOGIC_BASE_URL").unwrap_or_default().trim().to_string(),
        api_key: std::env::var("FORMLOGIC_API_KEY").unwrap_or_default().trim().to_string(),
    };
    ...
    // Clean shutdown: stop every running plugin + service on SIGTERM / Ctrl-C.
    {
        let registry = registry.clone();
