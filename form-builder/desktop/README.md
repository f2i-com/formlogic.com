...
The web app polls `http://127.0.0.1:17872/api/health` on load to detect
...
All routes are bound to `127.0.0.1:17872`. CORS is `*` since the bind
is loopback-only.
...
```pwsh
curl http://127.0.0.1:17872/api/health
```
Expected response:
```json
...
`17872` is fixed for now. It's high enough to avoid permission issues
