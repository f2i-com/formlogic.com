...
| onSubmit `logicScript` (`forms.logic_script`) | **server** — ZIPP sandbox (the `formlogic-runtime` launcher, built from ZIPP source) in the submission pipeline | trusted + authoritative — the one surface a client cannot bypass | validation, computed/hidden fields, rejecting bad data (test via `POST /api/forms/{formId}/script/test`) |
