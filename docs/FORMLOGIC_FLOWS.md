...
```
input, output, condition, template, logic_block (ZIPP-sandboxed expression, statements or function body),
...
| `condition` | `expr` (sandboxed JS bool: same code styles as `logic_block`), `timeoutMs?` | Branches on a sandboxed boolean. Errors (incl. a budget overrun) fail the run. Wall clock capped (1s default; `timeoutMs` 100ms–30s overrides — same bounds as `logic_block`). | routes the **True**/**False** handle |
| `template` | `template` | Interpolates `{{path}}` into a string. | string → `$nodes.<id>` |
| `logic_block` | `expr` (sandboxed JS: an expression, statements ending in the result, or a function body that `return`s a value), `timeoutMs?`, `scope?` | Runs JS in the ZIPP sandbox over a frozen `{inputs,event,app,nodes,upstream,kv}`. Errors (incl. a budget overrun) fail the run — same as `condition`. Wall clock capped (2s default; `timeoutMs` 100ms–30s overrides). | your return value → `$nodes.<id>` |
