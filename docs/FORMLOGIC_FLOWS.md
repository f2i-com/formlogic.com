...
```
input, output, condition, template, logic_block (ZIPP-sandboxed JavaScript: expression, statements or function body; or Python, §4.4),
...
**In the browser, user-authored flow code runs in the ZIPP sandbox.** The node types that evaluate author-supplied *code* are `condition` and `logic_block`, and in the browser runner BOTH run in the `lib/formlogic` ZIPP engine (`engine.ts` → `formlogic.worker.ts` → `zipp-host.ts` over `ui/vendor/zipp-wasm`, installed from the Softn release: real JS on a locked-down interpreter; `eval`/`new Function` appear nowhere in `client-runtime/flows/*` — a vitest static scan asserts this). `template`/`selectors` are pure string/path operations, never a code path. FormLogic Cloud refuses both node types. Two exceptions are real and not ZIPP: **(1)** `browser_action` with `action: 'evaluate'` sends its `script` to the local Playwright Browser service, which runs it inside the automated page with that page's DOM and privileges (§4.3) — by design, and outside the sandbox's limits; **(2)** a run claimed by a paired desktop (`runtime: 'desktop'`) is executed by OAIY Desktop, which from OAIY v0.0.5 runs that code on ZIPP as well: it hands the graph's `condition`/`logic_block` code, each app's logic scripts and every binding condition to its bundled `oaiy` CLI as leaf scripts on one warm engine, under the prelude it fetches from this server's `GET /api/v1/script-profile` and verifies for itself. Given no prelude it refuses its linked lanes and drops its engine claim, so the work is kept and run in the browser rather than run bare. Its own restricted condition evaluator now only shadows the engine, to log where the two disagree (see [FORMLOGIC_DESKTOP.md](FORMLOGIC_DESKTOP.md)). `logic_block` code may be a single expression (its value is the node's output), statements whose last expression is the result (`let r = inputs.x * 2; r`), or a function body that `return`s a value (`const c = …; return { found: !!c };`); a `condition` node accepts an expression or statements whose last value is the result, but not a function body — a top-level `return` fails there as it does for form expressions — while a binding's `condition` stays a plain expression. Either node may instead be written in Python (`data.language: 'python'`, §4.4), which only the browser runner executes. A `logic_block` receives a **frozen, JSON-only** `ctx` exposing exactly `{inputs, event, kv, app}` (plus `nodes`/`upstream` for graph plumbing): `kv` is a **read-only snapshot** of the node's KV scope (`data.scope`, else `flow:<slug>`), so a logic block can *read* KV inline but can only *write* via a capability-gated `storage_set`/`formlogic.store`. It runs under a **2 s default wall-clock timeout** (`data.timeoutMs` overrides within 100 ms…30 s) so a wedged evaluation can never stall the run budget.
...
| `condition` | `expr` (sandboxed JS bool: an expression or statements ending in it, no top-level `return`; or ONE Python expression, §4.4), `language?`, `timeoutMs?` | Branches on a sandboxed boolean (Python: by truthiness). Errors (incl. a budget overrun) fail the run. Wall clock capped (1s default; `timeoutMs` 100ms–30s overrides — same bounds as `logic_block`). | routes the **True**/**False** handle |
| `template` | `template` | Interpolates `{{path}}` into a string. | string → `$nodes.<id>` |
| `logic_block` | `expr` (sandboxed JS: an expression, statements ending in the result, or a function body that `return`s a value; or Python: an expression, or statements that set `result`, §4.4), `language?`, `timeoutMs?`, `scope?` | Runs JS or Python in the ZIPP sandbox over a frozen `{inputs,event,app,nodes,upstream,kv}`. Errors (incl. a budget overrun) fail the run — same as `condition`. Wall clock capped (2s default; `timeoutMs` 100ms–30s overrides). | your return value → `$nodes.<id>` |
...
- **`browser_action`** → the bundled `playwright-browser` service (default :17880). One node runs a full step: (re)use a session (thread `$nodes.<id>.sessionId` into a later browser action to reuse the same page), optionally navigate (`url`) + wait (`waitFor`), then perform the `action`. `extract_text` runs `innerText` (of `selector`, else the body); `evaluate` runs `script` in the page — in that page's own JavaScript engine, not the ZIPP sandbox.
...
### 4.4 Python code nodes (formlogic-python/1)

`condition` and `logic_block` nodes may be written in Python: `data.language` is `'python'`. Absent (or `null` / `''`) means `'javascript'`, so every stored graph keeps its meaning; any other value is refused when the flow is saved (400) and fails a run `invalid_flow`. The editor's **Language** select sets it, and switching keeps the code as written. The code stays in `data.expr`. The contract (`ui/src/lib/formlogic/python/pythonContract.ts`, id `formlogic-python/1`) is pinned on the installed engine by `docs/contracts/formlogic-python-logic-corpus.json`.

- **`logic_block`**: ONE expression, or statements that set a top-level `result` (the node's output; `None` when never set). Python has no top-level `return`; code with one is a syntax error. The block is first compiled as one bracketed expression, `(` + your code + `)`, and only when that does not compile, as statements. So code that is valid inside brackets runs as an expression even across lines: `inputs["a"]` on one line and `+ inputs["b"]` on the next is `inputs["a"] + inputs["b"]` (the brackets join the lines), where as statements the second line would be a separate, discarded expression. And statements never return their last expression: `x = 1` then `x * 2` gives `None`, because only `result` counts (`x = 1` then `result = x * 2` gives `2`).
- **`condition`**: ONE expression (it may span lines), judged by Python truthiness (`[]`, `{}`, `""` and `0` are false). Statements are an error.
- **Names**: `inputs`, `event`, `app`, `nodes`, `upstream` and (for `logic_block`) `kv` hold the same values as for JavaScript, as dicts and lists, so subscript every level: `nodes["lookup"]["found"]`, `inputs["from"]` (attribute access does not work). The editor's "Insert a value" chips write that form. `print` does nothing, as the JavaScript console does.
- **Helpers**: FormLogic's `formlogic.py` ports the prelude: `validators`, `compliance`, `finance`, `safety`, `is_empty`, `is_not_empty`, `contains`, `avg`. `format.*` is JavaScript-only; Python's own `sum` and `len` stand in for `sum` and `count`. ZIPP's Python is a subset (the same on v0.0.18 and v0.0.19): `json`, `re`, `math`, `statistics`, `collections` and `itertools` import; `datetime`, `decimal`, `uuid`, `base64`, `urllib.parse` and `csv` do not.
- **Result**: JSON data only. A non-finite float becomes `None` and a tuple a list; an int beyond ±(2^53 − 1), a set, bytes or an object is an error naming the type.
- **Errors** name the author's own lines (`KeyError: 'missing' (line 2)`) and fail the run exactly as a JavaScript error does. One exception: an error inside an expression that spans lines (a `condition`, or a `logic_block` that runs as one expression) reports **line 1**, whichever line of the expression raised it; statements report the line itself.
- **Where it runs**: only the browser runner. Its Worker loads ZIPP web-python and gives every Python evaluation a fresh engine, the same instruction budget and the same 2 s / 1 s wall clocks (which now include about 50–80 ms of Python start-up). FormLogic Cloud refuses code nodes in either language, and the server sandbox has no Python. A Desktop runs Python only once it follows the whole contract in [FORMLOGIC_DESKTOP.md](FORMLOGIC_DESKTOP.md) §8 (advertise `logic-language:python`, and declare its languages on reserve, claim, the relay claim, the graph fetch and the app-logic catalogue); until then the language gate (§10) keeps Python runs away from it and the browser runs them. The Test Run drawer's **Run via Desktop relay** is disabled, with the reason, for a flow with Python code unless the target computer advertises it.
- **Packages**: a contributed node whose core preset lowers it to a `condition` or `logic_block` takes the preset's `defaults.language` (the node's own `data.language` wins, as for every default). The gate reads that lowered language, not just the stored graph, and a preset may default only to `'javascript'` or `'python'` (a package with any other value does not install).
- **Known limit — a stale editor tab**: an editor tab still running a FormLogic bundle from before Python runs **Test Run in browser** locally and ignores `data.language`, so it would run a Python block as JavaScript. Reload open editor tabs after upgrading. Live runs are not affected: an old tab's reserve and claim send no `logicLanguages` and are refused, and an old app tab receives no Python app-logic scripts (§10, [CUSTOM_APP_PLATFORM.md](CUSTOM_APP_PLATFORM.md#python-scripts-formlogic-python1)).
- **Binding conditions stay JavaScript** (`flow-binding.schema.json`), and so does a pack connector's demo driver. App-logic scripts may be Python too (`def run(ctx)`, see [CUSTOM_APP_PLATFORM.md](CUSTOM_APP_PLATFORM.md#app-logic-zipp-sandbox)).
...
- **Logic languages**: `condition` and `logic_block` nodes may declare `data.language` — `'javascript'` (the default when absent) or `'python'`; saving any other value is a 400. A runtime built before Python ignores that field and would run Python as JavaScript, so reserve, claim and the queued listings take the caller's `logicLanguages` (a JSON list on reserve/claim, `?logicLanguages=javascript,python` on the listings). Absent means JavaScript only: a flow with Python code is refused with **409** `{code: 'language_unsupported', languages}` before anything is written, and its queued runs are left out of that caller's listing (before the `LIMIT`, so they never hold its queue head). A `queued: true` reserve is the exception — its reserver runs nothing, so it is accepted whatever it declares (an OAIY plugin event always reserves queued, and must not be lost while no FormLogic tab is open); the claim is what is gated. `GET /api/v1/flows` (the graph a Desktop runs) takes the same `?logicLanguages=` and leaves out flows it would misread, and the E2E relay queues a Python flow only for a Desktop advertising `logic-language:python` and refuses a relay claim that does not declare it. A flow's languages are those of what it executes: a package core preset that lowers to Python counts. The browser declares both. For desktop-first events it defers a binding to a fresh Desktop only when that Desktop's heartbeat capabilities include `logic-language:<id>` for every non-JavaScript language the flow (and the flows it calls) has code in; otherwise the browser runs it. Binding conditions stay JavaScript.
...
`onSubmit(ctx)` scripts can trigger flows: `ctx.flows.run(slug, input?)` **records an intent** (it never executes a flow inside the sandbox) and returns `{queued: boolean, error?}` to the script. The ZIPP runtime guest (`formlogic/runtime/guest`) exposes `ctx.flows` as a host-RPC module; the PHP side (`FormLogicRuntime` → `DbContextCapture::addFlowRun`) validates and caps the intents, and `ScriptResult.flowRuns` carries them out of the sandbox.
