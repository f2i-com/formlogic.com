...
- **Runner.** A WorkflowGraph interpreter with the SAME node semantics as the browser executor (`input/output/condition/template/logic_block/llm_chat/http_request/formlogic_*|connector_request/storage_*/aokie_speak` + the desktop-service nodes `browser_action/image_gen/stt_transcribe/tts_speak`), a 50-node budget, per-run timeout, and a 50-node cap. User code (`condition`/`logic_block`) runs in the vendored static `qjs` binary (bundled as an embedded resource; 2 s / 64 MiB caps). `http_request`/`llm_chat` are allow-listed to the FormLogic base URL + local loopback services only.
  *Script engine today.* The `qjs` binary above is the design as frozen; before its retirement FormLogic Desktop replaced it with an embedded ZIPP sandbox. OAIY Desktop, which now fills this role, runs on ZIPP as well, from OAIY v0.0.5. It reserves runs on events and executes them only after claiming them, and it hands claimed runs, each app's `onConnectorEvent` scripts and every binding condition to its bundled `oaiy` CLI as leaf scripts on one warm engine, kept alive behind a line protocol rather than spawned per evaluation. It fetches this server's leaf-script profile from `GET /api/v1/script-profile`, verifies the digest itself, and carries the prelude into every batch, so an expression means here what it means in the browser. Given no profile it REFUSES its linked lanes rather than running them bare, and drops its engine claim in the same breath, so the work is kept and run in the browser instead of quietly losing every script that calls a prelude helper. Its own restricted condition evaluator still runs, but only beside the engine and only to log where the two disagree; the engine decides.
  *Which OAIY, and on whose word.* FormLogic does not take a CLI on trust. `scripts/fetch-oaiy-cli.mjs` resolves an OAIY release ONCE and freezes its identity — the same rule the Softn fetcher follows, so a release published mid-run cannot change what a later job installs — then refuses unless the downloaded bytes match the digest the release itself states, the release's own `SHA256SUMS.txt` covers the asset, `release-evidence-linux.json` names that commit as verified and audited by the run that attested it, the archive's inner manifest accounts for every file with no extra file and no symlink, the packed `oaiy-cli.json` matches what that evidence says the release carries, the engine is `ready`, its languages include Python, and it speaks every protocol this FormLogic speaks (`formlogic/ui/src/lib/oaiy/protocol.json`). What the evidence gives is a STRUCTURAL attestation, not a signature. A release carrying no CLI asset is refused outright rather than walked back to an older one, which would silently adopt an engine nobody chose. The `oaiy-parity` CI job then drives both shipped corpora through that CLI and compares every answer against this tree's own reading, after asserting that the two sides are even running the same engine build.
...
- **Logic languages (formlogic-python/1) — the contract a Desktop (OAIY) follows before it runs Python.** `condition` and `logic_block` nodes, and app-logic scripts, may be Python (`data.language` / a script's `language` is `'python'`; absent, `null` or `''` is JavaScript). A runtime built before Python ignores that field and would run Python as JavaScript — sometimes "successfully" (`result = inputs["n"] // 2` is `result = inputs["n"]` plus a comment) — so the server withholds Python work from any caller that does not say it runs Python, at every step. A Desktop that runs Python must do **all** of the following; one it skips keeps Python work from reaching it at that step (a relay run it cannot claim stays pending until it expires):
  ...
  Language needs are computed from what a flow **executes**: a package core preset whose default lowers a contributed node to a Python `logic_block` counts as Python (the server reads the lowered graph and the revision's compiled IR). Having declared Python, the Desktop must run it as formlogic-python/1 on ZIPP web-python — the files, wrappers and line offsets in `ui/src/lib/formlogic/python/pythonContract.ts`, served as `python.modes` of the leaf-script profile below and pinned by `docs/contracts/formlogic-python-logic-corpus.json` — and must fail a node whose language it does not know (`invalid_flow`), never run it as JavaScript. Binding conditions stay JavaScript.
...
- **The leaf-script profile (`GET /api/v1/script-profile`) — FormLogic's semantics as data, not as a second implementation.** Declaring `logic-language:python` says the Desktop *can* run Python; the profile is what tells it what a FormLogic expression **means**. It carries the standard library every expression sees (`prelude.js` — `validators.*`, `format.*`, `compliance.*`, `finance.*`, `safety.*`, `sum`, `avg`, `count`, …), its sha256, the instruction budget the browser host uses, and the `formlogic-python/1` contract's shared guest module. Without it a flow condition calling `validators.email(x)` throws "is not defined" on the Desktop and passes in the browser — the exact silent divergence the ecosystem exists to prevent. The route is documented in `docs/API.md`; in short:
  ...
  - **Body** — OAIY's `ScriptProfile` exactly: `{v: 1, preamble, preambleSha256, instructionSteps, python: {contract, files, entry, call, modes}}`. That schema (`oaiy.com protocol/v1/script-profile.schema.json`) is `additionalProperties: false` at the top level *and* inside `python`, so there is no envelope and no room for one: the document's id is the path, its revision is the `ETag`, and budgets come from the Desktop's connector descriptor.
  ...
  - **`python.modes` — the five wrappings, so a consumer can run the logic at all.** The shared contract (`formlogic.py`, `entry`, `call`) says what every evaluation carries; a mode says how one KIND of author code is wrapped before it runs. Each carries the entry module it adds (`files: {"main.py": …}` — `pythonContract.ts` `ENTRIES`), the file the author's source goes in (`block: "logic_block.py"`), the text `before` and `after` it (`BLOCK_WRAPPERS`), the count of generated lines that precede the author's first (`lineOffset` — `LINE_OFFSETS`), and, for `syntax` alone, its own `call` (`__formlogic_compiled__`, which is what that entry defines). A job names the modes to try in order and carries the author's text unwrapped, with no `files`/`entry`/`call`/`fallbackOnSourceError` beside them (`src/lib/oaiy/scriptJob.ts`; the schema refuses all four). The runner merges, wraps, runs, and **subtracts that mode's `lineOffset` from any line the engine blames**, so a Desktop reports `KeyError: 'missing' (line 1)` against the author's own numbering without re-deriving the arithmetic. What it must still do for itself is what FormLogic does in `authorMessage`: drop the driver's frames (`main.py`, `formlogic.py`) and rename `logic_block.py:2` to `line 2`. `syntax` deserves a note, because it used to be a divergence: a runner calls `mode.call` for every mode, and that entry's function was an `import logic_block`, so a check that exists to answer "does this parse?" RAN the author's code on the one path and not on the other. It no longer does. The block is compiled when `initPythonProject` compiles the module the entry imports - that compile IS the check, and a block that does not parse fails initialisation with the author's line - while the function a runner calls does not import it. Both paths therefore answer `null` for a block that compiles, whatever it would have done, and both report the same `SyntaxError` for one that does not (`src/lib/oaiy/profileRunnerMirror.test.ts` holds both, in the same test). What `syntax` still does not tell you is anything about what the block would DO: it is a parse, not a dry run.
  - **Only guest-side data crosses.** The driver stays in FormLogic: engine construction and `setInstructionBudget`, `initPythonProject`, which modes a kind tries and why, the reply channel, the output sanitiser and the author-message rewrite. A test asserts the served payload names none of them. A mode's wrapper text and entry module are guest code and do cross — that is the point of them — so `__formlogic_value__` and `__formlogic_condition__` are in the document and no host identifier is.
