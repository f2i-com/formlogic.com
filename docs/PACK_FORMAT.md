...
`linked_record.properties.targetFormId`, `navConfig[].formId`, `settings.landingPage`, and role
`permissions[].packFormId`. On import these are remapped to the freshly-created form UUIDs. A `linked_record`
pointing at an unknown packFormId is a hard error; nav/landing refs to unknown forms are dropped.
...
`customScreen` ≤ 512KB (app `navConfig`/`settings`/`theme` ≤ 10KB). Oversized packs are rejected with a clear
message.
