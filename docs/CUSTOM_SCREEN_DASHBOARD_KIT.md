...
> .ts, then: `node scripts/emit-marketplace.mjs` → `php bin/provision-demo.php` (see below).
...
2. `node scripts/emit-marketplace.mjs` (ui/) → `php bin/provision-demo.php` (backend/; `RESEED_DEMO=1` to regenerate data).
